...
```{r BApred,echo=FALSE,warning=FALSE,message=FALSE,fig.height=5,fig.width=7}
...
for(Sp in spIds) 
{
  ...
  # add parameters
  train_sp[,
    c('psi', 'sigma_plot', 'Beta') := post_mean[
    species_id == Sp,
    .(psi, sigma_plot, Beta)
  ]
  ]
  ...
  train_sp[
    plotEffect |>
      ...
      select(-species_id) |>
      setDT(),
    psiPlot := log(i.psi_natural_mean),
    on = 'plot_id'
  ]

  # predict longetivity rate
  train_sp[,
    longev_log := 1/(1 + exp(
        -psiPlot +
        BA_plot * Beta
  ...
  p1 <- train_sp |>
      filter(!is.na(mort_pred)) |>
      select(BA_plot, mort, mort_pred) |>
      pivot_longer(
        cols = contains('mort'),
        names_to = 'sim'
      ) |>
      ggplot(aes(x = BA_plot, y = value, color = sim)) +
        geom_jitter(alpha = .4, size = .2) +
        geom_smooth(method = 'gam') +
        xlab('Plot basal area') +
        ylab('Mortality events') +
        labs(subtitle = Sp) +
        theme_minimal() +
        geom_function(
          fun = function(x)
            1 - (1/(1 + exp(-post_sp$psi + BA_plot * post_sp$Beta))^train_sp[, median(deltaYear)]),
          color = 1,
          linetype = 'dashed'
        )
...
```{r looLog, echo = FALSE, warning=FALSE,fig.height = 8, fig.width = 8, warning=FALSE, message=FALSE}
map_dfr(
    spIds,
    ~ map2(
        setNames(sim_toLoad, sim_toLoad),
