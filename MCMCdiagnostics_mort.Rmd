---
 ...
 subtitle: "Test v14.0 - plot random effect - Plot basal area fixed effect"
 author: "Will Vieira"
...
post |>
  ...
  group_by(species_id) |>
  expand_grid(BA_plot = seq(0, 120, 5)) |>
  mutate(
    BAeffect = BA_plot * Beta,
    psi_BA = exp(psi + BAeffect)
  ) |>
  select(iter, species_id, BA_plot, BAeffect, psi_BA) |>
  pivot_longer(
    cols = c('BAeffect', 'psi_BA'),
    names_to = 'sim'
  ) |>
  mutate(
    sim = factor(sim, levels = c('BAeffect', 'psi_BA'), labels = c('Plot basal area effect (log linear)', 'Plot basal area effect on longetivity'))
  ) |>
  ggplot(aes(x = BA_plot, y = value, color = species_id)) +
...
## Mortality in function of plot Basal area

```{r plot_preds,echo=FALSE,warning=FALSE,message=FALSE,fig.height=5,fig.width=8}
...
for(Sp in spIds)
{
  ...
  # Prediction figures
  p1 <- train_sp |>
    select(iter2, BA_plot, mort, mort_pred) |>
    mutate(
      BAc = cut(
        BA_plot,
        seq(min(BA_plot), max(BA_plot), length.out = 10),
        include.lowest = TRUE
      )
    ) |>
    group_by(BAc, iter2) |>
    ...
    ggplot(aes(x = BAc, y = value, color = name)) +
      ...
      geom_function(
        fun = function(x)
          1 - ((1/(1 + exp(-(post_sp['psi'] + x * post_sp['Beta'])))^train_sp[, median(deltaYear)])),
        color = 1,
        linetype = 'dashed'
      ) +
      labs(color = '')
