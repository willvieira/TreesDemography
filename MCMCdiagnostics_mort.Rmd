---
 ...
 subtitle: "Test v18.1 - plot and year random effect - Size, basal area competition, temperature, and precipitation fixed effect"
 author: "Will Vieira"
...
mort_dt[
  nbMeasure > 1,
  temp_sc := (bio_01_mean - min(bio_01_mean))/(max(bio_01_mean) - min(bio_01_mean)),
  by = species_id
]
# fill scaled temperature NAs for tree_ids with nbMeasure == 1
mort_dt[,
  temp_sc := nafill(nafill(temp_sc, "locf"), "nocb"),
  by = .(species_id, plot_id, year0)
]
# scale precipitation
mort_dt[
  nbMeasure > 1,
  prec_sc := (bio_12_mean - min(bio_12_mean))/(max(bio_12_mean) - min(bio_12_mean)),
  by = species_id
]
# fill scaled precipitation NAs for tree_ids with nbMeasure == 1
mort_dt[,
  prec_sc := nafill(nafill(prec_sc, "locf"), "nocb"),
  by = .(species_id, plot_id, year0)
]
  ...
  bind_rows(
    ...
    mutate(
      value = case_when(
        ...
        par == 'optimal_temp' ~ rbeta(nIter, 2, 2),
        par == 'tau_temp' ~ rnorm(nIter, 0, 2),
        par == 'optimal_prec' ~ rbeta(nIter, 2, 2),
        par == 'tau_prec' ~ rnorm(nIter, 0, 2)
      )
    )
  )
...
## Longevity in function of time

```{r deltaYearPred,echo=FALSE,warning=FALSE, message=FALSE,fig.height = 6.5, fig.width = 7}
post |>
  filter(par == 'psi') |>
  pivot_wider(
    names_from = par,
  ...
  ggplot(aes(x = deltaYear,  y = long, color = species_id)) +
...
```{r SizeEffect,echo=FALSE,warning=FALSE,message=FALSE,fig.height=7,fig.width=10}
post |>
...
```{r BAeffect,echo=FALSE,warning=FALSE,message=FALSE,fig.height=10,fig.width=10}
post |>
...
```{r temp_pred,echo=FALSE,fig.height = 7, fig.width = 10}
post |>
  filter(par %in% c('psi', 'optimal_temp', 'tau_temp')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  group_by(species_id) |>
  expand_grid(bio_01_mean = seq(0, 1, 0.05)) |>
  mutate(
    tempEffect = -tau_temp * (bio_01_mean - optimal_temp)^2,
    psi_T = exp(psi + tempEffect)
  ) |>
  select(iter, species_id, bio_01_mean, tempEffect, psi_T) |>
  pivot_longer(
    cols = c('tempEffect', 'psi_T'),
    names_to = 'sim'
  ) |>
  mutate(
    sim = factor(sim, levels = c('tempEffect', 'psi_T'), labels = c('Effect of MAT on the longevity parameter', 'Longevity in function of MAT'))
  ) |>
...
```{r prec_pred,echo=FALSE,fig.height = 7, fig.width = 10}
post |>
  filter(par %in% c('psi', 'optimal_prec', 'tau_prec')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  group_by(species_id) |>
  expand_grid(prec_sc = seq(0, 1, .05)) |>
  mutate(
    precEffect = -tau_prec * (prec_sc - optimal_prec)^2,
    psi_P = exp(psi + precEffect)
  ) |>
  select(iter, species_id, prec_sc, precEffect, psi_P) |>
  pivot_longer(
    cols = c('precEffect', 'psi_P'),
    names_to = 'sim'
  ) |>
  mutate(
    sim = factor(sim, levels = c('precEffect', 'psi_P'), labels = c('Effect of MAP on the longevity parameter', 'Longevity in function of MAP'))
  ) |>
  ggplot(aes(x = prec_sc, y = value, color = species_id)) +
    stat_lineribbon(.width = 0.9, alpha = .8) +
    facet_wrap(~sim, scales = 'free') +
    scale_fill_brewer() +
...
```{r plot_preds,echo=FALSE,warning=FALSE,message=FALSE,fig.height=12,fig.width=12}
...
for(Sp in spIds)
{
  ...
  # predict for random effects only model (mort_plotYear)
  # to compare predictions with and without fixed effects
  ##########################################################
    # merge global parameters
  train_sp[
    readRDS(paste0('output/mort_plotYear/posteriorPop_', Sp, '.RDS')) |>
      pivot_wider(names_from = par, values_from = value) |>
      setDT(),
    psi_re := i.psi,
    on = 'iter'
  ...
  # merge plot_id parameters if plot is present in validation data
  train_sp[
    readRDS(paste0('output/mort_plotYear/posteriorpsiPlot_', Sp, '.RDS')) |>
      mutate(
        plot_id_seq = parse_number(par)
      ) |>
      select(!par) |>
  ...
  # merge plot_id parameters if plot is present in validation data
  train_sp[
    readRDS(paste0('output/mort_plotYear/posteriorpsiYearInt_', Sp, '.RDS')) |>
      mutate(
        time_int = parse_number(par)
      ) |>
      select(!par) |>
  ...
  # compute longevity logit
  train_sp[, longev_log_re := 1/(1 + exp(-(psi_re + psiPlot_re + psiYear_re)))]

  # compute mortality rate in function of time
  train_sp[, mortality_prob_re := 1 - (longev_log_re^deltaYear)]

  # predict mortality event
  train_sp[,
    mort_pred_re := rbinom(.N, size = 1, mortality_prob_re)
  ]
  ...
  dt_p1 <- train_sp |>
    select(iter2, BA_comp_sp, mort, mort_pred, mort_pred_re) |>
    mutate(
      BAc = cut(
        BA_comp_sp,
    ...
    summarise(
      ...
      Pred_re = sum(mort_pred_re == 1)/n()
    ) |>
    pivot_longer(
      cols = c('Obs', 'Pred', 'Pred_re')
    )

  dt_p2 <- train_sp |>
    select(iter2, BA_comp_intra, mort, mort_pred, mort_pred_re) |>
    mutate(
      BAc = cut(
        BA_comp_intra,
...
# Prediction figures
  dt_p3 <- train_sp |>
    select(iter2, dbh0, mort, mort_pred, mort_pred_re) |>
    mutate(
      sizec = cut(
        dbh0,
  ...
  dt_p4 <- train_sp |>
    select(iter2, bio_01_mean, mort, mort_pred, mort_pred_re) |>
    mutate(
      tempc = cut(
        bio_01_mean,
        seq(min(bio_01_mean), max(bio_01_mean), length.out = 10),
        include.lowest = TRUE
      )
    ) |>
    group_by(tempc, iter2) |>
    summarise(
      Obs = sum(mort == 1)/n(),
      Pred = sum(mort_pred == 1)/n(),
      Pred_re = sum(mort_pred_re == 1)/n()
  ...
  dt_p5 <- train_sp |>
    select(iter2, bio_12_mean, mort, mort_pred, mort_pred_re) |>
    mutate(
      precc = cut(
        bio_12_mean,
        seq(min(bio_12_mean), max(bio_12_mean), length.out = 10),
        include.lowest = TRUE
      )
    ) |>
    group_by(precc, iter2) |>
    summarise(
      Obs = sum(mort == 1)/n(),
      Pred = sum(mort_pred == 1)/n(),
      Pred_re = sum(mort_pred_re == 1)/n()
  ...
  yLim = range(c(dt_p1$value, dt_p2$value, dt_p3$value, dt_p4$value, dt_p5$value))
  ...
  p4 <- dt_p4 |>
    ggplot(aes(x = tempc, y = value, color = name)) +
      geom_boxplot(alpha = .4) +
  ...
  p5 <- dt_p5 |> 
    ggplot(aes(x = precc, y = value, color = name)) +
      geom_boxplot(alpha = .4) +
      xlab('Scaled mean annual precipitation') +
      ylab('Mortality proportion') +
      labs(subtitle = '') +
      theme_minimal() +
      labs(color = '') + 
