---
 ...
 subtitle: "Test v16.0 - plot random effect - mean annual precipitation fixed effect"
 author: "Will Vieira"
...
```{r plot_preds,echo=FALSE,warning=FALSE,message=FALSE,fig.height=5,fig.width=10}
...
for(Sp in spIds)
{
  ...
  mort_prob = function(psi) return( 1 - ( (1/(1 + exp(-(psi))))^2  ) )
  ...
  p1 <- train_sp |>
    ...
    ggplot(aes(x = tempc, y = value, color = name)) +
      ...
      # geom_function(
      #   fun = function(x)
      #     mort_prob(post_sp['tau_prec'] * (x - post_sp['optimal_prec'])^2),
      #   color = 1,
      #   linetype = 'dashed'
      # ) +
      labs(color = '')
...
mortProp_ls |>
  ...
  ggplot(aes(x = mortProp_pred, y = species_id)) +
    ...
    theme_classic() +
    xlab('Mortality rate (obs in red vs predicted in grey)') +
    ylab('')
```
