---
 ...
 subtitle: "Test v13.0 - plot random effect - Size fixed effect"
 author: "Will Vieira"
...
```{r BApred,echo=FALSE,warning=FALSE,message=FALSE,fig.height=7,fig.width=7}
...
post_mean <- post |>
  filter(
    par %in% c('psi', 'sigma_plot', 'size_opt', 'size_var')
  ) |>
...
for(Sp in spIds) 
{
  ...
  # add parameters
  train_sp[,
    c('psi', 'sigma_plot', 'size_opt', 'size_var') := post_mean[
                      species_id == Sp,
                      .(psi, sigma_plot, size_opt, size_var)
                    ]
  ]
  ...
  p1 <- train_sp |>
      ...
      ggplot(aes(x = dbh0, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            1 - 1/(1 + exp(-post_sp$psi - ((log(x/post_sp$size_opt)/post_sp$size_var)^2)^train_sp[, median(deltaYear)])),
          color = 1,
          linetype = 'dashed'
        )
