...
# Prediction figures
  ...
  p2 <- dt_p2 |>
    ggplot(aes(x = BAc, y = value, color = name)) +
      ...
      geom_function(
        fun = function(x)
          1 - ((1/(1 + exp(-(post_sp['psi'] + x * post_sp['Beta'] * post_sp['theta'])))^train_sp[, median(deltaYear)])),
        color = 1,
        linetype = 'dashed'
      ) +
      labs(color = '') +
  ...
  p3 <- dt_p3 |>
    ggplot(aes(x = sizec, y = value, color = name)) +
      ...
      geom_function(
        fun = function(x)
          1 - ((1/(1 + exp(-(post_sp['psi'] - (log(dbh0/post_sp['size_opt'])/post_sp['size_var'])^2))))^train_sp[, median(deltaYear)]),
        color = 1,
        linetype = 'dashed'
      ) +
      labs(color = '')
