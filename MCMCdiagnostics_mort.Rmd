---
 ...
 subtitle: "Test v8.0 - random effect"
 author: "Will Vieira"
 date: "`r paste('Last updated on', format(Sys.time(), '%d %B, %Y'))`"
---


## Test if plot random effects on baseline longevity improve predictions:

- **sim1**: no random effect
- **sim2**: random effect on psi
...
```{r get MCMC sampling data, echo = F}
  ...
  mcmcSampleDF_random <- subset(mcmcSampleDF, pars %in% grep('psi_pmean', unique(mcmcSampleDF$pars), value = TRUE))
```
...
# Plot of size, temperature, precipitation and competition effect
  ...
  # define functions of the model
    ...
    compEffect = function(xba, param, sim) {
        return(param['Lo', 1] + ((1 - param['Lo', 1]) / (1 + exp(0.25 * (xba - param['Mid', 1])))))
    }
...
```{r calculate OOB predictions and confusion matrix, echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  mort_model2 <- function(param, sim, plot_id, D, TP, PP, C)
  {
    if(sim == 'sim1') {
      psi_pop <- param["psi"]
    }else{
      psi_pop <- param["psi"] + param[paste0("psi_pmean[", plot_id, "]")]
    }
  ...
  for(sp in spIds)
  {
    ...
    for(sim in simulations)
    {
      ...
      for(i in 1:sampleN)
      {
        ...
        # get mortality
        db[, mortByYear := 1 - mort_model2(param = pars, sim = sim, plot_id = plot_id, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, C = BA)]
