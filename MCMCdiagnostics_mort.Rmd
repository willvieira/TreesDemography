---
 ...
 subtitle: "Test v17.1 - plot random effect - Size, basal area competition, temperature and precipitation  fixed effect"
 author: "Will Vieira"
...
post |>
  ...
  group_by(species_id) |>
  expand_grid(prec_sc = seq(0, 1, 0.05)) |>
  mutate(
...
for(Sp in spIds)
{
  ...
  ##########################################################
    # merge global parameters
  train_sp[
    readRDS(paste0('output/mort_plot/posteriorPop_', Sp, '.RDS')) |>
      pivot_wider(names_from = par, values_from = value) |>
      setDT(),
    psi_re := i.psi,
    on = 'iter'
  ...
  # merge plot_id parameters if plot is present in validation data
  train_sp[
    readRDS(paste0('output/mort_plot/posteriorpsiPlot_', Sp, '.RDS')) |>
      mutate(
        plot_id_seq = parse_number(par)
      ) |>
      select(!par) |>
  ...
  # compute longevity logit
  train_sp[, longev_log_re := 1/(1 + exp(-(psi_re + psiPlot_re)))]
  # compute mortality rate in function of time
