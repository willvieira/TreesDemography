---
 ...
 subtitle: "Test v15.0 - plot random effect - mean annual temperature fixed effect"
 author: "Will Vieira"
...
post |>
  ...
  group_by(species_id) |>
  expand_grid(temp_sc = seq(0, 1, .05)) |>
  mutate(
    tempEffect = -tau_temp * (temp_sc - optimal_temp)^2,
    psi_T = exp(psi + tempEffect)
  ) |>
  select(iter, species_id, temp_sc, tempEffect, psi_T) |>
  pivot_longer(
  ...
  ggplot(aes(x = temp_sc, y = value, color = species_id)) +
...
for(Sp in spIds)
{
  ...
  # Prediction figures
  p1 <- train_sp |>
    select(iter2, temp_sc, mort, mort_pred) |>
    mutate(
      tempc = cut(
        temp_sc,
        seq(min(temp_sc), max(temp_sc), length.out = 10),
        include.lowest = TRUE
      )
    ) |>
    ...
    ggplot(aes(x = tempc, y = value, color = name)) +
      ...
      xlab('Scaled mean annual temperature') +
      ylab('Mortality proportion') +
      labs(subtitle = Sp) +
      theme_minimal() +
      # geom_function(
      #   fun = function(x)
      #     mort_prob(post_sp['tau_temp'] * (x - post_sp['optimal_temp'])^2),
      #   color = 1,
      #   linetype = 'dashed'
      # ) +
      labs(color = '')
