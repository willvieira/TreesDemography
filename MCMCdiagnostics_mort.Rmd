---
 ...
 subtitle: "Test v11.0 - plot and individual random effect"
 author: "Will Vieira"
...
posteriorpsiTree_files <- grep(
  'posteriorpsiTree',
  dir(file.path('output', simName), full.names = TRUE),
  value = TRUE
)
names(posteriorpsiTree_files) <- names(sort(sapply(spIds, function(x) grep(x, posteriorpsiTree_files))))
...
plotEffect_ls <- treeEffect_ls <- list()
for(Sp in spIds)
{
    psi_pop <- post |>
    ...
    dt_toTree <- readRDS(posteriorpsiTree_files[Sp]) |>
      ...
      treeEffect_ls[[Sp]] <- dt_toTree |>
          ...
          filter(iter == 1) |>
          select(
            tree_id, tree_id_seq, psi_pop, psi_natural_mean, dbh0, BA_comp, bio_01_mean
          )
      ...
      dt_toTree <- dt_toTree |>
          ...
          left_join(
            dt_toPlot |>
              select(plot_id, iter, plot_id_seq, psiPlot)
          ) |>
          arrange(plot_id_seq) |>
          mutate(
            psi_naturalPlotTree = exp(value + psi_pop + psiPlot)
          )
...
treeEffect |>
  select(species_id, tree_id, psi_pop, psi_natural_mean) |>
  saveRDS(
    file.path(
        'output', 
        simName,
        'meanTreeEffect_psi.RDS'
    )
  )
```
...
```{r sizePred,echo=FALSE,warning=FALSE,warning=FALSE,message=FALSE,fig.height = 6.5, fig.width = 7}
post |>
  filter(par == 'psi') |>
  mutate(long = exp(value)) |>
  ggplot(aes(x = long, y = species_id)) +
    ggridges::geom_density_ridges2(alpha = 0.8, color = NA) +
    theme_minimal() +
    xlab('Longevity (years)')
```
...
for(Sp in spIds)
{
  ...
  # merge plot_id parameters if plot is present in validation data
  train_sp[
    readRDS(posteriorpsiTree_files[Sp]) |>
      mutate(
        tree_id_seq = parse_number(par)
      ) |>
      select(!par) |>
      setDT(),
    psiTree := i.value,
    on = c('iter', 'tree_id_seq')
  ]
...
for(Sp in spIds_val)
{
  ...
  val_sp[
    is.na(plot_id_seq),
    psiPlot := rnorm(.N, 0, sigma_plotTree * p_plotTree)
  ]

  # As the sample was based on the individual ID, there will not be any tree id present both in the training and validation db
  # So simply simulate from the random variable
  val_sp[,
    psiTree := rnorm(.N, 0, sigma_plotTree * (1 - p_plotTree))
  ]
