...
filter(species_id %in% simInfo$spIds)
```
...
pred_data |>
  ...
  geom_text_repel(
    data = pred_data |>
      ...
      mutate(
        max_x = max(BA_comp),
        max_y = mean(r_comp[BA_comp == max_x])
      ) |>
      slice_head(n = 1),
    mapping = aes(x = max_x, y = max_y, label = species_name),
    max.overlaps = Inf,
...
post |>
  ...
  expand_grid(BA_comp = seq(0, 100, 5)) |>
  mutate(
    r_comp = exp(r + Beta * theta * BA_comp)
  ) ->
pred_data
...
post |>
  ...
  group_by(species_name) |>
  expand_grid(clim = seq(0, 1, 0.05)) |>
  mutate(
    r_clim = exp(r + -tau_temp * (clim - optimal_temp)^2)
  ) ->
pred_data
...
pred_data |>
  ...
  theme_classic() +
  labs(
    x = 'Mean annual temperature (normalized)',
    y = 'Growth rate parameter',
    color = ''
  ) +
  geom_text_repel(
...
post |>
  ...
  expand_grid(clim = seq(0, 1, 0.05)) |>
  mutate(
    r_clim = exp(r + -tau_prec * (clim - optimal_prec)^2)
  ) ->
pred_data
...
pred_data |>
  ...
  theme_classic() +
  labs(
    x = 'Mean annual precipitation (normalized)',
    y = 'Growth rate parameter',
    color = ''
  ) +
  geom_text_repel(
...
```{r crossValidation, echo=FALSE, warning=FALSE, message=FALSE, fig.height=14,fig.width=9}
...
for(Sp in spIds)
{
  ...
  # filter some observations to reduce plot time
  ...
  p5 <- val_sp[
      deltaTime > 0,
      .(
        m_obs = mean(obs_growth),
        m_pred = mean(pred_growth_random),
        BA_comp_sp = mean(BA_comp_sp)
      ),
      by = obsID
    ] |>
    mutate(
      BAc = cut(BA_comp_sp, seq(min(BA_comp_sp), max(BA_comp_sp), length.out = 10), include.lowest = TRUE)
    ) |>
    pivot_longer(cols = contains('m_')) |>
    mutate(name = gsub('m_', '', name)) |>
    ggplot() +
    aes(BAc, value) +
    aes(fill = name) +
    geom_boxplot() +
  ...
  p6 <- val_sp[
      ...
      .(
        m_obs = mean(obs_growth),
        m_pred = mean(pred_growth_random),
        BA_comp_inter = mean(BA_comp_inter)
      ),
      by = obsID
    ] |>
    mutate(
      BAc = cut(BA_comp_inter, seq(min(BA_comp_inter), max(BA_comp_inter), length.out = 10), include.lowest = TRUE)
    ) |>
    pivot_longer(cols = contains('m_')) |>
    mutate(name = gsub('m_', '', name)) |>
    ggplot() +
  ...
  p7 <- val_sp[
      ...
      .(
        m_obs = mean(obs_growth),
        m_pred = mean(pred_growth_random),
        clim = mean(bio_01_mean_scl)
      ),
      by = obsID
    ] |>
    mutate(
      climc = cut(clim, seq(min(clim), max(clim), length.out = 10), include.lowest = TRUE)
    ) |>
    pivot_longer(cols = contains('m_')) |>
    mutate(name = gsub('m_', '', name)) |>
    ggplot() +
...
map_dfr(
  ...
  ggplot() +
  aes(x = value, y = fct_reorder(species_name, value), fill = sim) +
...
```{r plot Rsquared_gelman,echo=FALSE,fig.height = 7, fig.width = 6, warning=FALSE, message=FALSE}
map_dfr(
    sim_toLoad,
