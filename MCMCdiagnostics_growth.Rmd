---
 ...
 subtitle: "Test v5.0 - Test two complementary function for the competition effect: Basal area for plot competition and canopyDistance for individual competition"
 author: "Will Vieira"
...
growth_dt <- growth_dt[, .(sp_code2, growth, dbh0, dbh1, deltaYear, value5_bio60_01, value5_bio60_12, BA, canopyDistance)]

# transform dt
growth <- tidyr::gather(data = growth_dt,
            growth, dbh0, value5_bio60_01, value5_bio60_12, BA, canopyDistance,
            key = 'var', value = 'value')
```
...
# Plot of size, temperature, precipitation and competition effect
  ...
  # define functions of the model
    ...
    compEffect = function(xBA, xCD, sigma_C, Mid, beta, sim) {
      if(sim == 'sim1') {
        canopyDistanceEffect <- (0 + ((1 - 0)/(1 + exp(-0.2 * (xCD - Mid)))))     
        return( canopyDistanceEffect + ((1 - canopyDistanceEffect) * exp(-((xBA^2)/2 * sigma_C^2))) )
      }else{
        canopyDistanceEffect <- (0 + ((1 - 0)/(1 + exp(-beta * (xCD - Mid)))))     
        return( canopyDistanceEffect + ((1 - canopyDistanceEffect) * exp(-((xBA^2)/2 * sigma_C^2))) )
      }
    }

    canopyDistanceEffect = function(xCD, Mid, beta, sim) {
      if(sim == 'sim1') {
        return( (0 + ((1 - 0)/(1 + exp(-0.2 * (xCD - Mid))))) )
      }else{
        return( (0 + ((1 - 0)/(1 + exp(-beta * (xCD - Mid))))) )
      }
    }
  ...
  tempx = seq(-8, 25)
  precx = seq(500, 1700)
  BAx = seq(0, 80, length.out = 400)
  CDx = seq(-20, 30, length.out = 400)

  # species specfic colours
  spCols <- rev(RColorBrewer::brewer.pal(length(spIds), "Set3"))

  par(mfrow = c(5, length(simulations)), xaxs = "i", las = 1, mar = c(3, 1, 1.5, 1.5), oma = c(1,3.5,2,.2), mgp = c(1.5, 0.3, 0), tck = -.008, cex.lab = 1.2, cex.axis = 1.2)
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(BAx, compEffect(BAx, xCD = -20, sigma_C, Mid, beta, sim), xlim = c(0, 80),
      ylim = c(0, 1), type = 'l', xlab = 'Basal area (m2/ha)', ylab = 'Competition effect',
      col = spCols[sp],
      yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)

      points(BAx, compEffect(BAx, xCD = -20, sigma_C, Mid, beta, sim), type = 'l', col = spCols[sp])
    }
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(CDx, canopyDistanceEffect(CDx, Mid, beta, sim), xlim = c(-20, 30),
      ylim = c(0, 1), type = 'l', xlab = 'Canopy distance (m)', ylab = 'canopyDistance effect',
      col = spCols[sp],
      yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)

      points(CDx, canopyDistanceEffect(CDx, Mid, beta, sim), type = 'l', col = spCols[sp])
    }
  ...
  #     z <- outer(BAx, CDx, compEffect, sigma_C = sigma_C, Mid = Mid, Lo = Lo,  sim = sim)
  #     fields::image.plot(BAx, CDx, z)
  #     if(sp == 1) plot(CDx, compEffect(xBA = 15, CDx, sigma_C, Mid, Lo, sim), xlim = c(-20, 30),
  #     ylim = c(0, 1), type = 'l', xlab = 'Canopy distance (m)', ylab = 'Competition effect',
  #     col = spCols[sp],
  #     yaxt = ifelse(sim == 'sim1', "s", "n"))
  #     if(sim == 'sim2') axis(2, labels = FALSE)

  #     points(CDx, compEffect(xBA = 15, CDx, sigma_C, Mid, Lo, sim), type = 'l', col = spCols[sp])
  #   }
...
# Plot predictions
  ...
  growth_model <- function(param, sim, newdata, indVar = FALSE)
  {
    ...
    if(sim == 'sim1') {
      canopyDistanceEffect <- (0 + ((1 - 0)/(1 + exp(-0.2 * (newdata$CD - param["Mid"])))))
    }else {
      canopyDistanceEffect <- (0 + ((1 - 0)/(1 + exp(-param["beta"] * (newdata$CD - param["Mid"])))))
    } 

    competition <- canopyDistanceEffect + ((1 - canopyDistanceEffect) * exp(-((newdata$BA^2)/2 * param["sigma_C"]^2)))
  ...
  plot_prediction <- function(sim, sp, model, data) {
    ...
    if(sim == 'sim1')
      par(mfcol = c(5, length(simulations) * 2), xaxs="i", las = 1, mar = c(3, 1, 3, 1.5), oma = c(1,3.5,2,.2), mgp = c(1.5, 0.3, 0), tck = -.008, cex.lab = 1.2, cex.axis = 1.2)
    ...
    for(indVar in c(FALSE, TRUE))
    {
      ...
      plot(growth ~ BA, growth_ci10C, type = "n", ylim = c(0, 4),
          xlab = "Basal area (m²/ha)", ylab = "", yaxt = ifelse(sim == 'sim1', "s", "n"), cex.lab = 1.2)
      axis(2, labels = F)
      ...
      # Growth vs canopyDistance

      # get predictions
      if(!indVar)
      {
        growth_meanCD <- growth_model(param = param_mean, sim = sim, newdata = newdataCD)
        growth_ci10CD <- growth_model(param = param_ci10, sim = sim, newdata = newdataCD)
        growth_ci90CD <- growth_model(param = param_ci90, sim = sim, newdata = newdataCD)
      }else {
        DF <- growth_model(param = param_mean, sim = sim, newdata = newdataCD, indVar = TRUE)
        growth_meanCD <- DF[, c('means', varNames)]
        growth_ci10CD <- DF[, c('lower', varNames)]
        growth_ci90CD <- DF[, c('upper', varNames)]
        names(growth_meanCD)[1] <- names(growth_ci10CD)[1] <- names(growth_ci90CD)[1] <- 'growth'
      }

      plot(growth ~ CD, growth_ci10CD, type = "n", ylim = c(0, 4),
          xlab = "Canopy distance (m)", ylab = "", yaxt = ifelse(sim == 'sim1', "s", "n"), cex.lab = 1.2)
      axis(2, labels = F)
      for(ba in ba2plot) {
        mu <- subset(growth_meanCD, BA == ba)
        ci10 <- subset(growth_ci10CD, BA == ba)
        ci90 <- subset(growth_ci90CD, BA == ba)
        mycol <- ifelse(indVar, colba_indVar[ba2plot == ba], colba[ba2plot == ba])

        lines(growth ~ CD, mu, cex = .5, pch = 19, col = mycol)
  ...
  for(sp in spIds) {
    for(sim in simulations) {
      ...
      # Basal area
      newdataCD <- data.frame(expand.grid(BA = c(5, 20, 40),
                                        CD = seq(-20, 35, length.out = 300),
                                        TP = 4,
                                        PP = 1000,
                                        D = 200))
      
      plot_prediction(sim, sp, growth_model,
      data = list(newdataT = newdataT, newdataP = newdataP, newdataD = newdataD, newdataC = newdataC, newdataCD = newdataCD))
    }
  }
  #
#
...
```{r calculate OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  # growth model
  growth_model2 <- function(param, sim, D, TP, PP, BA, CD, indVar = FALSE)
  {
    ...
    # competition function depending on simulation
    if(sim == 'sim1') {
      canopyDistanceEffect <- (0 + ((1 - 0)/(1 + exp(-0.2 * (CD - param["Mid"])))))
    }else {
      canopyDistanceEffect <- (0 + ((1 - 0)/(1 + exp(-param["beta"] * (CD - param["Mid"])))))
    } 
    
    competition <- canopyDistanceEffect + ((1 - canopyDistanceEffect) * exp(-((BA^2)/2 * param["sigma_C"]^2)))
    ...
    if(!indVar)
    {
      ...
      lower <- qgamma(.95, (mu_d^2/param["sigma_base"]), mu_d/param["sigma_base"], lower.tail = FALSE)
      upper <- qgamma(.95, (mu_d^2/param["sigma_base"]), mu_d/param["sigma_base"])
      growth <- list(means = mu_d, lower = lower, upper = upper)
    }
  ...
  for(sp in spIds)
  {
    for(sim in simulations)
    {
      ...
      if(sim == 'sim1')
      {
        ...
        oob_list[[sp]][, Gdet_mean_sim1 := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci10_sim1 := growth_model2(param = param_ci10, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim1 := growth_model2(param = param_ci90, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]

        # get growth rate with individual variation
        oob_list[[sp]][, c('Gvar_mean_sim1', 'Gvar_ci10_sim1', 'Gvar_ci90_sim1') := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = TRUE)]
      }else{
          # get deterministic growth rate
        oob_list[[sp]][, Gdet_mean_sim2 := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci10_sim2 := growth_model2(param = param_ci10, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim2 := growth_model2(param = param_ci90, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]

        # get growth rate with individual variation
        oob_list[[sp]][, c('Gvar_mean_sim2', 'Gvar_ci10_sim2', 'Gvar_ci90_sim2') := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = TRUE)]
      }
    }
  }
...
```{r plot OOB predictions,echo=FALSE,fig.height = 8, fig.width = 12}
  ...
  # load random forest fitted curve of real vs predicted growth
  predCurveRF <- readRDS('data/quebec/randomForest_predCurve.RDS')
  
  # prepare table to save lm() info from the simulations
  n <- length(spIds) * (length(simulations) + 1)
  reg_dt <- data.table(sp_code2 = rep(spIds, (length(simulations) + 1)), sim = character(n), R2 = numeric(n), Slope = numeric(n))
  ...
  for(sp in spIds)
  {
    ...
    reg_sim1 <- lm(Gdet_mean_sim1 ~ growth, data = oob_growth[sp_code2 == sp])
    reg_sim2 <- lm(Gdet_mean_sim2 ~ growth, data = oob_growth[sp_code2 == sp])
  
    # add to reg table
    reg_dt[sp_code2 %in% sp, `:=`(sim = c(simulations, 'randomForest'),
                                  R2 = c(summary(reg_sim1)$r.squared, summary(reg_sim2)$r.squared, predCurveRF[[sp]][3, 'R2']),
                                  Slope = c(reg_sim1$coefficients[[2]], reg_sim2$coefficients[[2]], predCurveRF[[sp]][3, 'Slope']))]

    # legend
    leg <- c('1:1 line',
              paste0('lm() from randomForest: R2 = ', round(predCurveRF[[sp]][3, 'R2'], 2), '; slope = ', round(predCurveRF[[sp]][3, 'Slope'], 2)),
              paste0('lm() from Bayesian sim1: R2 = ', round(summary(reg_sim1)$r.squared, 3), '; slope = ', round(reg_sim1$coefficients[['growth']], 3)),
              paste0('lm() from Bayesian sim2: R2 = ', round(summary(reg_sim2)$r.squared, 3), '; slope = ', round(reg_sim2$coefficients[['growth']], 3)))
    
    for(sim in simulations)
    {
      # sample 10% of data
      dbAll <- oob_growth[sp_code2 == sp, c(2, grep(sim, names(oob_growth))), with = FALSE]
      ...
      abline(predCurveRF[[sp]][3, 'Intercept'], predCurveRF[[sp]][3, 'Slope'], lty = 3)
      if(sim == 'sim1') legend('topleft', legend = leg, lty = c(1, 2, 3, 3), cex = 2.8, bty = 'n')
...
reg_dt %>%
  mutate(sp_code2 = forcats::fct_reorder(sp_code2, R2)) %>% 
  ggplot(mapping = aes(x = sp_code2, y = R2, fill = sim, color = sim)) +
  geom_point() +
  coord_flip()
```
...
Density distribution draws from the posterior distribution ($n = 500$) can be compared with the MSE values using the random forest methods (vertical bars).
...
```{r calculate MSE, echo = FALSE}
  ...
  # growth model
  growth_model3 <- function(param, sim, D, TP, PP, BA, CD, indVar = FALSE)
  {
  ...
  MSE_dt <- setDT(data.frame(sp = character(), sim = integer(), MSE_det = logical(), MSE_var = numeric(), r2 = numeric()))
  sampleN <- 500

  for(sp in spIds)
  {
    
    db <- oob_growth[sp_code2 == sp, .(growth, dbh0, value5_bio60_01, value5_bio60_12, BA, canopyDistance)]
    ...
    for(sim in simulations)
    {
      ...
      params <- lapply(posterior, function(x) sample(x, sampleN))
      ...
      for(i in 1:sampleN)
      {
        ...
        {
          # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]
          # get growth rate with individual variation
          db[, g_var_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = TRUE)]
        }else{
            # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, CD = canopyDistance, indVar = FALSE)]
          # get growth rate with individual variation
      ...
      # update main dt
      MSE_dt <- rbind(MSE_dt, data.frame(sp = rep(sp, sampleN), sim = rep(sim, sampleN), MSE_det = MSE_det, MSE_var = MSE_var, r2 = r2))
    }
  }
...
```{r plot MSE,echo=FALSE,fig.height = 6, fig.width = 8, warning=FALSE, message=FALSE}
  ...
  rf_dt <- readRDS('data/quebec/randomForest_growth.RDS')
  rf_dt$RandomForest <- plyr::revalue(rf_dt$model, c('m1' = 'sim1', 'm2' = 'sim2', 'mFull' = 'sim_full'))
  
  # For deterministic growth
  ggplot(data = MSE_dt, aes(MSE_det, sp, fill = sim, colour = sim)) + 
    geom_density_ridges(alpha = 0.4, scale= 1.8) +
    geom_segment(data = subset(rf_dt, RandomForest == 'sim1'), aes(linetype = `RandomForest`, x = MSE, xend = MSE, y = length(sp):1,
                                      yend = (length(sp):1) + .8),
               color = "red", alpha = 0.6) +
    geom_segment(data = subset(rf_dt, RandomForest == 'sim2'), aes(linetype = `RandomForest`, x = MSE, xend = MSE, y = length(sp):1,
                                  yend = (length(sp):1) + .8),
            color = "#4aaab0", alpha = 0.6) +
    geom_segment(data = subset(rf_dt, RandomForest == 'sim_full'), aes(linetype = `RandomForest`, x = MSE, xend = MSE, y = length(sp):1,
                                  yend = (length(sp):1) + .8),
  ...
  # For ind variation growth
    ggplot(data = MSE_dt, aes(MSE_var, sp, fill = sim, colour = sim)) + 
      geom_density_ridges(alpha = 0.4, scale= 1.8) +
      geom_segment(data = subset(rf_dt, RandomForest == 'sim1'), aes(linetype = `RandomForest`, x = MSE, xend = MSE, y = length(sp):1,
                                        yend = (length(sp):1) + .8),
                color = "red", alpha = 0.6) +
...
```{r plot Rsquared,echo=FALSE,fig.height = 6, fig.width = 8, warning=FALSE, message=FALSE}
  ...
  # For deterministic growth
  ggplot(data = MSE_dt, aes(r2, sp, fill = sim, colour = sim)) + 
    geom_density_ridges(alpha = 0.4, scale= 1.8) +
    geom_segment(data = subset(rf_dt, RandomForest == 'sim1'), aes(linetype = `RandomForest`, x = Rsquared, xend = Rsquared, y = length(sp):1,
                                      yend = (length(sp):1) + .8),
               color = "red", alpha = 0.6) +
    geom_segment(data = subset(rf_dt, RandomForest == 'sim2'), aes(linetype = `RandomForest`, x = Rsquared, xend = Rsquared, y = length(sp):1,
                                  yend = (length(sp):1) + .8),
            color = "#4aaab0", alpha = 0.6) +
    geom_segment(data = subset(rf_dt, RandomForest == 'sim_full'), aes(linetype = `RandomForest`, x = Rsquared, xend = Rsquared, y = length(sp):1,
                                  yend = (length(sp):1) + .8),
            color = "black", alpha = 0.6) +
    xlim(0, 0.64) +
    theme_classic() +
