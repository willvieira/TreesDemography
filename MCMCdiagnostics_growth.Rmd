---
 ...
 subtitle: "Test v17.0 - Von Bertalanffy model - r in function of BA_comp. Individual random effects on `r`"
 author: "Will Vieira"
...
for(sp_seq in seq(1, 17, 2)) 
{
  ...
  for(i in 0:1)
  {
    ...
    train_sp[, r_plotTreeBA := exp((log(r_plot) + log(r_tree) - log(r_pop)) + BA_comp * beta)]
    
    # predict size
    train_sp[,
      pred_size := dbh0 *
                  exp(-r_plotTreeBA * deltaTime) +
                  Lmax * (1 -exp(-r_plotTreeBA * deltaTime))
    ]
...
for(Sp in spIds)
{
  ...
  val_sp[,
    r_plot := exp(r + rPlot_log + rTree_log + BA_comp * beta)
  ]
