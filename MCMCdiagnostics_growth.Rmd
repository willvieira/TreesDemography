...
```{r plot MSE,echo=FALSE,fig.height = 8, fig.width = 12, warning=FALSE}

  # For deterministic growth
  ggplot(MSE_dt) + 
    geom_density(aes( x = MSE_det, colour = sim, fill = sim), alpha = 0.5) +
    facet_wrap(~ sp, scales = c('free_y')) +
    theme_classic() +
    xlim(0.5, 9) +
  ...
  ggplot(MSE_dt) + 
    geom_density(aes( x = MSE_var, colour = sim, fill = sim), alpha = 0.5) +
    facet_wrap(~ sp, scales = c('free_y')) +
    theme_classic() +
    xlim(.5, 9) +
