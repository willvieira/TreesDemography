---
 ...
 subtitle: "Test v4.0 - Test two complementary function for the competition effect: Basal area for plot competition and canopyDistance for individual competition"
 author: "Will Vieira"
...
- **sim1**: split competition effect between BA and canopyDistance [pdg * (0.5 * BA + 0.5 * canopyDistance)]
- **sim2**: Aditional competition effect with canopyDistance [pdg * BA * canopyDistance]
...
# Load simulation variables
  ...
  sim1name <- 'Splitted competition'
  sim2name <- 'Aditinal competition'
...
growth_dt <- growth_dt[, .(sp_code2, growth, dbh0, value5_bio60_01, value5_bio60_12, BA, canopyDistance)]
...
# Plot of size, temperature, precipitation and competition effect
  ...
  # define functions of the model
    ...
    compEffect = function(xBA, xCD, sigma_C, Mid, Lo, sim) {
      if(sim == 'sim1') {
        return((0.5 * exp(-((xBA^2)/2 * sigma_C^2)) + 0.5 * (Lo + ((1 - Lo)/(1 + exp(-0.5 * (xCD - Mid)))))))
      }else{
        return(exp(-((xBA^2)/2 * sigma_C^2)) * (Lo + ((1 - Lo)/(1 + exp(-0.5 * (xCD - Mid))))))
      }
    }
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(BAx, compEffect(BAx, xCD = 0, sigma_C, Mid, Lo, sim), xlim = c(0, 80),
      ylim = c(0, 1), type = 'l', xlab = 'Basal area (m2/ha)', ylab = 'Competition effect',
      col = spCols[sp],
      yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)

      points(BAx, compEffect(BAx, xCD = 0, sigma_C, Mid, Lo, sim), type = 'l', col = spCols[sp])
    }
...
# Plot predictions
  ...
  growth_model <- function(param, sim, newdata, indVar = FALSE)
  {
    ...
    if(sim == 'sim1') {
      competition <- (0.5 * exp(-((newdata$BA^2)/2 * param["sigma_C"]^2))) + 0.5 * (param["Lo"] + ((1 - param["Lo"])/(1 + exp(-0.5 * (newdata$CD - param["Mid"])))))
    }else {
      competition <- exp(-((newdata$BA^2)/2 * param["sigma_C"]^2)) * (param["Lo"] + ((1 - param["Lo"])/(1 + exp(-0.5 * (newdata$CD - param["Mid"])))))
    }
...
```{r calculate OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  growth_model2 <- function(param, sim, D, TP, PP, BA, CD, indVar = FALSE)
  {
    ...
    # competition function depending on simulation
    if(sim == 'sim1') {
      competition <- (0.5 * exp(-((BA^2)/2 * param["sigma_C"]^2))) + 0.5 * (param["Lo"] + ((1 - param["Lo"])/(1 + exp(-0.5 * (CD - param["Mid"])))))
    }else {
      competition <- exp(-((BA^2)/2 * param["sigma_C"]^2)) * (param["Lo"] + ((1 - param["Lo"])/(1 + exp(-0.5 * (CD - param["Mid"])))))
    }
