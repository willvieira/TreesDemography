...
```{r calculate MSE, echo = FALSE}
  ...
  # growth model
  growth_model3 <- function(param, sim, D, TP, PP, C, BA, indVar = FALSE)
  {
  ...
  for(sp in spIds)
  {
    ...
    db <- oob_growth[sp_code2 == sp, .(growth, dbh0, value5_bio60_01, value5_bio60_12, canopyDistance, BA)]
    ...
    for(sim in simulations)
    {
      ...
      for(i in 1:sampleN)
      {
        ...
        {
          # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, C = canopyDistance, BA = BA, indVar = FALSE)]
          # get growth rate with individual variation
          db[, g_var_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, C = canopyDistance, BA = BA, indVar = TRUE)]
        }else{
            # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, C = canopyDistance, BA = BA, indVar = FALSE)]
          # get growth rate with individual variation
