---
 ...
 subtitle: "Test v2.0 - Improve competition (basal area) function"
 author: "Will Vieira"
...
# Plot of size, temperature, precipitation and competition effect
  ...
  # define functions of the model
    ...
    compEffectBA = function(xBA, Lo, beta, mid) {
      return( (Lo + ((1 - Lo) / (1 + exp(-beta * (xBA - mid))))) )
    }
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(BAx, compEffectBA(BAx, Lo, beta, mid), xlim = c(0, 80),
      ylim = c(0, 1), type = 'l', xlab = 'Canopy distance', ylab = 'Competition effect',
      col = spCols[sp],
      yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)

      points(BAx, compEffectBA(BAx, Lo, beta, mid), type = 'l', col = spCols[sp])
    }
...
# Plot predictions
  ...
  growth_model <- function(param, sim, newdata, indVar = FALSE)
  {
    ...
    if(sim == 'sim1') {
      ...
      competition <- (param["Lo"] + ((1 - param["Lo"]) / (1 + exp(0.3 * (newdata$CD - param["mid"])))))
    }
...
```{r calculate OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  growth_model2 <- function(param, sim, D, TP, PP, BA, indVar = FALSE)
  {
    ...
    if(sim == 'sim1') {
      ...
      competition <- (param["Lo"] + ((1 - param["Lo"]) / (1 + exp(0.3 * (BA - param["mid"])))))
    }
...
```{r plot OOB predictions,echo=FALSE,fig.height = 8, fig.width = 12}
  ...
  for(sp in spIds)
  {
    ...
    for(sim in simulations)
    {
      db <- oob_growth[sp_code2 == sp, c(2, grep(sim, names(oob_growth))), with = FALSE]
