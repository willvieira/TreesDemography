---
 ...
 subtitle: "Test v16.1 - Von Bertalanffy model - r in function of BA_comp. Using previous measure as S_t instead of using the first measurement of the individual - RESEF run"
 author: "Will Vieira"
...
treeData[, .(spCode, dbh, deltaTime, sampled2)] |>
  mutate(sample = sampled2) |>
  pivot_longer(
    cols = c(dbh, deltaTime),
    names_to = 'var'
  ) |>
  ggplot(aes(x = value, y = spCode, fill = sample)) +
...
post |>
  ...
  select(iter, spCode, BA_comp, BAeffect, r_BA) |>
  pivot_longer(
  ...
  ggplot(aes(x = BA_comp, y = value, color = spCode)) +
    stat_lineribbon(.width = 0.95, alpha = .8) +
...
for(Sp in spIds)
{
  val_sp <- trainData[spCode == Sp & sampled == 'validation']
