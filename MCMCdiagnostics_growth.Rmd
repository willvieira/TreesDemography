...
```{r BApred,echo=FALSE, warning=FALSE, message=FALSE,fig.height = 10, fig.width = 10}
...
post_mean <- post |>
  filter(
    par %in% c('r', 'Lmax', 'Beta', 'optimal_temp', 'sigma_temp', 'optimal_prec', 'sigma_prec')
  ) |>
...
for(Sp in spIds) 
{
  ...
  p1 <- train_sp |>
      ...
      select(dbh0, obs_growth, pred_growth) |>
      pivot_longer(
