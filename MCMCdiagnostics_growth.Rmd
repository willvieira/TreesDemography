...
post_mean <- post |>
  filter(
    par %in% c('r', 'Lmax', 'Beta', 'optimal_temp', 'tau_temp', 'optimal_prec', 'tau_prec')
  ) |>
...
for(Sp in spIds) 
{
  ...
  # add parameters
  train_sp[,
    c('Beta', 'Lmax', 'optimal_temp', 'tau_temp', 'optimal_prec', 'tau_prec') := post_mean[
                      species_id == Sp,
                      .(Beta, Lmax, optimal_temp, tau_temp, optimal_prec, tau_prec)
                    ]
  ]
