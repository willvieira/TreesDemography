    ...
    for(sim in simulations) {
      ...
      if(sim == 'sim1')
      {
        ...
        newdataC <- data.frame(expand.grid(BA = seq(0, 100, length.out = 300),
                ...
                # Temp
        newdataT <- data.frame(expand.grid(BA = c(-10, 0, 10),
                                          CD = 0,
                                          TP = seq(-10, 15, length.out = 300),
                                          PP = 1000,
                                          D = 200))
        # Prec
        newdataP <- data.frame(expand.grid(BA = c(-10, 0, 10),
                                          CD = 0,
                                          TP = 4,
                                          PP = seq(250, 1800, length.out = 300),
                                          D = 200))
        # Size
        newdataD <- data.frame(expand.grid(BA = c(-10, 0, 10),
                                          CD = 0,
                                          TP = 4,
                                          PP = 1000,
                                          D = seq(0, 900, length.out = 300)))
...
```{r calculate OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  growth_model2 <- function(param, sim, D, TP, PP, C, BA, plot_id, indVar = FALSE)
  {
    ...
    if(sim == 'sim1')
    {
      ...
      compEffect <- (params['Lo'] + ((1 - params['Lo']) / (1 + exp(-0.45 * (C)))))
    }
    ...
    for(sim in simulations)
    {
      ...
      if(sim == 'sim1')
      {
        ...
        oob_list[[sp]][, Gvar_ci90_sim1 := apply(summ_mat, 1, quantile, probs = 0.9, na.rm = TRUE)]
          ...
          # get deterministic growth rate
        oob_list[[sp]][, Gdet_mean_sim2 := growth_model2(param = param_mean, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDisstance, BA = BA, plot_id = plot_id_seq, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci10_sim2 := growth_model2(param = param_ci10, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDisstance, BA = BA, plot_id = plot_id_seq, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim2 := growth_model2(param = param_ci90, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDisstance, BA = BA, plot_id = plot_id_seq, indVar = FALSE)]
