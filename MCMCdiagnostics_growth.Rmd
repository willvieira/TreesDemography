...
post <- map2_dfr(
  ...
  ~ readRDS(.x) |>
    filter(par %in% c('r', 'sigma_obs', 'Lmax')) |>
    bind_cols(species_id = .y)
)
...
```{r plot Rsquared,echo=FALSE,fig.height = 7.5, fig.width = 8, warning=FALSE, message=FALSE}
map_dfr(
    sim_toLoad,
    ~ readRDS(file.path('output', .x, 'R2.RDS')) |>
      bind_cols(sim = .x)
  ) |>
  ggplot(aes(x = r2, y = species_id, fill = sim)) +
    geom_density_ridges(alpha = 0.8, scale= 1.8, color = NA) + xlim(0, .02)
```
