...
post |>
  ...
  group_by(species_id) |>
  expand_grid(time = seq(0, 2000, 60)) |>
...
```{r plot Rsquared_gelman,echo=FALSE,fig.height = 6, fig.width = 8, warning=FALSE, message=FALSE}
R2_list |>
    bind_rows() |>
    ggplot(aes(x = R2gelman_size, y = species_id)) +
...
R2_list |>
    ...
    select(species_id, iter2, contains('R2 predicted')) |>
    pivot_longer(
