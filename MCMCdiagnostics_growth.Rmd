...
```{r, echo = F, fig.height = 6, fig.width = 10}
  ...
  p <- ggplot(data = Rhat, aes(x = sp, y = Rhat)) +
       ...
       facet_grid(~sim, scales = 'free_x',
                 labeller = labeller(sim = setNames(c(sim1name, sim2name, sim3name),
                                     c('sim1', 'sim2', 'sim3'
                                    )))) +
       theme_classic()
  print(p)
...
```{r plot mcmcSample by pars, echo = F, message=FALSE, fig.height = 7, fig.width = 9}
  ...
  for(par in unique(mcmcSampleDF$pars))
  {
    p <- ggplot(data = subset(mcmcSampleDF, pars == par), aes(value, sp)) +
         ...
         facet_grid(~sim, scales = ifelse(par %in% c("pdg"), 'free_x', 'fixed'),
                   labeller = labeller(sim = setNames(c(sim1name, sim2name, sim3name),
                                       c('sim1', 'sim2', 'sim3')))) +
         theme_classic() +
         xlab(par)
...
growth_dt <- growth_dt[, .(species_id, growth, dbh0, mean_temp_period_3_lag, tot_pp_period3_lag, canopyDistance)]

# transform dt
growth <- tidyr::gather(data = growth_dt,
            growth, dbh0, mean_temp_period_3_lag, tot_pp_period3_lag, canopyDistance,
            key = 'var', value = 'value')
rm(growth_dt)
```
...
# Plot of size, temperature, precipitation and competition effect
  ...
  # define functions of the model
    ...
    # competition effect
    compEffect = function(xcanopy, Lo, Mid) {
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(canopyDist, compEffect(canopyDist, Lo, Mid), xlim = c(-35, 40),
      ylim = c(0, 1), type = 'l', xlab = 'Canopy distance', ylab = 'Competition effect',
      col = spCols[sp],
      yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)

      points(canopyDist, compEffect(canopyDist, Lo, Mid), type = 'l', col = spCols[sp])
    }
