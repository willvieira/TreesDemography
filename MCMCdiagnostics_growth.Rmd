---
 ...
 subtitle: "Test v18.0 - Von Bertalanffy model - r in function of `BA_comp` and `bio_01_mean`. Using previous measure as S_t instead of using the first measurement of the individual."
 author: "Will Vieira"
...
# prepare parameters
post_mean <- post |>
  filter(par %in% c('Lmax', 'Beta', 'optimal_temp', 'sigma_temp')) |>
...
for(Sp in spIds) 
{
  ...
  # add parameters
  train_sp[, c('Beta', 'Lmax', 'optimal_temp', 'sigma_temp') :=
    post_mean[species_id == Sp, .(Beta, Lmax, optimal_temp, sigma_temp)]]
  train_sp[
    plotEffect |>
