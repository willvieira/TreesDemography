---
 ...
 subtitle: "Test v16.1 - Von Bertalanffy model - r in function of BA. Using previous measure as S_t instead of using the first measurement of the individual - RESEF run"
 author: "Will Vieira"
...
# List output files and set vector names to spCode
...
treeData <- readRDS('data/RESEF/growth_dt.RDS')
treeData <- treeData[spCode %in% spIds]

# create plot_id_seq index to match stan output (ugly but wathever no time)
treeData_new <- data.table()
for(Sp in spIds)
{
  td_sp <- treeData[spCode == Sp]

  plot_id_uq <- td_sp[, unique(plot_id)]
  toSub <- data.table(
    plot_id = plot_id_uq,
...
for(Sp in spIds)
{
    ...
    dt_toPlot <- readRDS(posteriorrPlot_files[Sp]) |>
            ...
            left_join(
              treeData[ 
                ...
                .( 
                    ...
                    soil = unique(soil), 
                    BA = mean(BA), 
                    bio_01_mean = mean(value0_bio_01), 
                    bio_12_mean = mean(value0_bio_12) 
                ),
                by = plot_id_seq
              ]
            )
      ...
      dt_toPlot <- dt_toPlot |>
          mutate(plot_id_seq = fct_reorder(as.factor(plot_id_seq), latitude))
  
    print(
      ggarrange(
          dt_toPlot |>
              ggplot(aes(y = plot_id_seq, x = r_real, color = bio_01_mean)) +
                stat_pointinterval(alpha = .7, size = .5) +
                geom_vline(
                ...
                labs(subtitle = Sp),
          dt_toPlot |>
              ggplot(aes(y = plot_id_seq, x = r_real, color = BA)) +
...
r_pop <- plotEffect |>
  group_by(spCode) |>
  summarise(r = exp(mean(r_pop)))

print(
  plotEffect |>
    ggplot(aes(x = bio_01_mean, r_real, color = ecoregion)) +
        stat_pointinterval() +
...
print(
  plotEffect |>
    ggplot(aes(x = BA, r_real, color = bio_01_mean)) +
        stat_pointinterval() +
        facet_wrap(~spCode, scales = 'free') +
        geom_hline(data = r_pop, aes(yintercept = r), color = 'red') +
...
print(
  plotEffect |>
    ggplot(aes(x = bio_12_mean, r_real, color = BA)) +
        stat_pointinterval() +
        facet_wrap(~spCode, scales = 'free') +
        geom_hline(data = r_pop, aes(yintercept = r), color = 'red') +
...
print(
  plotEffect |>
    ggplot(aes(x = slope, r_real, color = drai)) +
        stat_pointinterval() +
        facet_wrap(~spCode, scales = 'free') +
        geom_hline(data = r_pop, aes(yintercept = r), color = 'red') +
        xlab('Plot slope') +
...
print(
  plotEffect |>
    ggplot(aes(x = soil, r_real, color = humus)) +
        stat_pointinterval() +
        facet_wrap(~spCode, scales = 'free') +
        geom_hline(data = r_pop, aes(yintercept = r), color = 'red') +
        xlab('Soil') +
...
plotEffect |>
  select(spCode, plot_id_seq, r_real) |>
  saveRDS(
    file.path(
        'output', 
        simName,
...
```{r organize data, echo = FALSE, warning=FALSE, message=FALSE, fig.height = 7, fig.width = 5}
# split between train and everything else (merge validate with rest of data)
treeData[, .(spCode, dbh0, deltaYear, BA)] |>
  pivot_longer(
    cols = c(dbh0, deltaYear, BA),
    names_to = 'var'
  ) |>
  ggplot(aes(x = value, y = spCode)) +
...
post |>
  ...
  ggplot(aes(x = time, y = Lt, color = spCode)) +
...
post |>
  ...
  group_by(spCode) |>
  expand_grid(BA = seq(0, 100, 5)) |>
  mutate(
    BAeffect = BA * beta,
    r_BA = exp(r + BAeffect)
  ) |>
  select(iter, spCode, BA, BAeffect, r_BA) |>
  pivot_longer(
    cols = c('BAeffect', 'r_BA'),
    names_to = 'sim'
  ) |>
  mutate(
    sim = factor(sim, levels = c('BAeffect', 'r_BA'), labels = c('BA effect on r parameter', 'r parameter'))
  ) |>
  ggplot(aes(x = BA, y = value, color = spCode)) +
    stat_lineribbon(.width = 0.9, alpha = .8) +
    facet_wrap(~sim, scales = 'free') +
    scale_fill_brewer() +
    xlab('Plot total basal area') +
    ylab('') +
    theme_minimal() +
...
for(sp_seq in seq(1, 16, 2)) 
{
  ...
  for(i in 0:1)
  {
    ...
    train_sp <- treeData[spCode == Sp]

    # sample few tree_ids (if nb of tree_ids are larget than 1000)
    # that have at least 2 measures
    nbMeasure <- train_sp[, .N, by = tree_id]
    if(nrow(nbMeasure) > 1000){
      tree_ids <- train_sp[, .N, by = tree_id][, sample(tree_id, 1000)]
      train_sp <- train_sp[tree_id %in% tree_ids]
    }

    # add parameters
    train_sp[, c('beta', 'Lmax') := post_mean[spCode == Sp, .(beta, Lmax)]]
    train_sp[
      plotEffect |>
        filter(spCode == Sp) |>
        select(-spCode) |>
    ...
    train_sp[, r_plotBA := exp(log(r_plot) + BA * beta)]
    
    # predict size
    train_sp[,
      pred_size := dbh0 *
                  exp(-r_plotBA * deltaYear) +
                  Lmax * (1 -exp(-r_plotBA * deltaYear))
    ]
    ...
    plot_ls[[i+1]] <- train_sp |>
        ...
        select(BA, obs_growth, pred_growth) |>
        pivot_longer(
          cols = contains('growth'),
          names_to = 'sim'
        ) |>
        ggplot(aes(x = BA, y = value, color = sim)) +
...
for(Sp in spIds)
{
  ...
  val_sp[,
    r_plot := exp(r + rPlot_log + BA * beta)
  ]

  # predict size
  val_sp[,
    pred_size := dbh0 *
                  exp(-r_plot * deltaYear) +
                  Lmax * (1 -exp(-r_plot * deltaYear))
  ]
  ...
  val_sp[, obs_growth := (dbh1 - dbh0)/deltaYear]
  val_sp[, pred_growth := (pred_size - dbh0)/deltaYear]
  val_sp[, pred_growth_random := (pred_size_random - dbh0)/deltaYear]

  # calculate MSE
  val_sp[, SE_size := (dbh1 - pred_size_random)^2]
  ...
  R2gel <- val_sp |>
    ...
    left_join(
      post |>
        filter(spCode == Sp & par == 'sigma_obs') |>
        mutate(sigma_obs = value) |>
        select(iter, sigma_obs, spCode)
    ) |>
    mutate(R2gelman_size = var_size/(var_size + sigma_obs^2)) |>
    select(iter, iter2, spCode, R2gelman_size) |>
    setDT()
  ...
  R2_list[[Sp]] <- merge(
    ...
    val_sp[,
      .(
        R2reg_size = get_r2(dbh1, pred_size_random),
        R2reg_growth = get_r2(obs_growth, pred_growth_random),
        R2reg_size_mean = get_r2(dbh1, pred_size),
  ...
  p1 <- val_sp[, .(m_obs = mean(dbh1), m_pred = mean(pred_size)), by = obsID] |>
...
sim_toLoad <- c(simInfo$simCompare, simName)
  ...
  ggplot(aes(x = value, y = spCode, fill = sim)) +
...
map_dfr(
  ...
  ggplot(aes(x = R2gelman_size, y = spCode, fill = sim)) +
      geom_density_ridges(alpha = 0.8, scale= 1.8, color = NA) +
...
map_dfr(
  ...
  select(spCode, iter2, sim, contains('R2 predicted')) |>
  pivot_longer(
...
map_dfr(
  ...
  ggplot(aes(x = icc, y = spCode, fill = sim)) +
      geom_density_ridges(alpha = 0.8, scale= 1.8, color = NA) +
      xlab('ICC') +
      theme_minimal()
...
map_dfr( 
  ...
  select(iter2, spCode, icc, R2reg_growth_mean) |>
  group_by(spCode) |>
  summarise(
    icc_mean = mean(icc),
  ...
  ggplot(aes(icc_mean, r2_mean, color = spCode)) +
    geom_point() +
...
map_dfr(
  ...
  select(spCode, sim, elpd_diff, se_diff) |>
  pivot_longer(
    cols = c(elpd_diff, se_diff),
    names_to = 'variable',
    values_to = 'value'
  ) |>
  ggplot(aes(x = value, y = spCode, color = sim)) +
    geom_point() +
...
ggplot(aes(x = total/3600, y = spCode)) +
  geom_boxplot() +
  xlab('Fit time (hours)') +
  theme_minimal()
```
