---
 ...
 subtitle: "Test v8.0 - canopyDistance effect with two different parameters set"
 author: "Will Vieira"
...
- sim1: sigmoid with `Lo` [0-1] and `mid` [0.1 - 0.5] parameters
- sim2: sigmoid with `Lo` [0-1] and `beta` [-10 - 10] parameters
...
# Load simulation variables
  ...
  sim1name <- 'Lo & mid pars'
  sim2name <- 'Lo & beta pars'
...
# Load rStan output and scaleInfo for all species_id
  ...
  simNames = list.files(path = 'MCMC', pattern = '.RDS', recursive = TRUE, full.names = TRUE)
...
growth <- growth_dt[, .(plot_id, species_id, growth, dbh0, deltaYear, mean_temp_period_3_lag, tot_annual_pp_lag, canopyDistance)]

# transform dt
growth <- tidyr::gather(data = growth,
            growth, dbh0, mean_temp_period_3_lag, tot_annual_pp_lag, canopyDistance,
            key = 'var', value = 'value')
```
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(tempx, tempEffect(tempx, topt, sigTopt), type = 'l',
                  ...
                  yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)
      xData = growth_dt[species_id == spIds[sp], seq(min(mean_temp_period_3_lag), max(mean_temp_period_3_lag), 0.1)]
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(precx, precEffect(precx, popt, sigPopt), type = 'l',
                  ...
                  yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)
      xData = growth_dt[species_id == spIds[sp], seq(min(tot_annual_pp_lag), max(tot_annual_pp_lag), 1)]
...
# Plot predictions
  ...
  growth_model <- function(param, sim, newdata, indVar = FALSE)
  {
    ...
    if(sim == 'sim1') {
      compEffect <- (param['Lo'] + ((1 - param['Lo']) / (1 + exp(-0.45 * (newdata$CD - param['mid'])))))
    }else{
      compEffect <- (param['Lo'] + ((1 - param['Lo']) / (1 + exp(-param['beta'] * (newdata$CD)))))
    }
    mu_d <- param["pdg"] *
            compEffect *
  ...
  plot_prediction <- function(sim, sp, model, data) {
    ...
    for(indVar in c(FALSE, TRUE))
    {
      ...
      for(ba in ba2plot) {
        mu <- subset(growth_meanT, CD == ba)
        ci10 <- subset(growth_ci10T, CD == ba)
        ci90 <- subset(growth_ci90T, CD == ba)
      ...
      if(indVar)
      {
        legend("topleft", legend = ba2plot, title = "Canopy distance",
      ...
      axis(2, labels = F)
      for(ba in ba2plot) {
        mu <- subset(growth_meanP, CD == ba)
        ci10 <- subset(growth_ci10P, CD == ba)
        ci90 <- subset(growth_ci90P, CD == ba)
        mycol <- ifelse(indVar, colba_indVar[ba2plot == ba], colba[ba2plot == ba])
      ...
      axis(2, labels = F)
      for(ba in ba2plot) {
        mu <- subset(growth_meanD, CD == ba)
        ci10 <- subset(growth_ci10D, CD == ba)
        ci90 <- subset(growth_ci90D, CD == ba)
        mycol <- ifelse(indVar, colba_indVar[ba2plot == ba], colba[ba2plot == ba])
      ...
      plot(growth ~ CD, growth_ci10C, type = "n", ylim = yLim,
      ...
      # add data range
      abline(v = growth_dt[species_id == sp, range(canopyDistance)], col = 'grey', lty = 2)
  ...
  for(sp in spIds)
  {
    for(sim in simulations)
    {
      ...
      if(sim == 'sim1')
      {
        ...
        oob_list[[sp]][, c('Gdet_mean_sim1', 'sizeEff_sim1', 'tempEff_sim1', 'precEff_sim1', 'compEff_sim1') := growth_model2(param = param_mean, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE, singleEffect = TRUE)]
        oob_list[[sp]][, Gdet_ci10_sim1 := growth_model2(param = param_ci10, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim1 := growth_model2(param = param_ci90, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
        ...
        for(i in 1:sampleN)
        {
          summ_mat[, i] <- oob_list[[sp]][, growth_model2(unlist(lapply(params, `[[`, i)), sim = sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = TRUE)]
        ...
        oob_list[[sp]][, Gvar_ci90_sim1 := apply(summ_mat, 1, quantile, probs = 0.9, na.rm = TRUE)]
          ...
          # get deterministic growth rate
        oob_list[[sp]][, c('Gdet_mean_sim2', 'sizeEff_sim2', 'tempEff_sim2', 'precEff_sim2', 'compEff_sim2') := growth_model2(param = param_mean, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE, singleEffect = TRUE)]
        oob_list[[sp]][, Gdet_ci10_sim2 := growth_model2(param = param_ci10, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim2 := growth_model2(param = param_ci90, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
...
```{r calculate MSE, echo = FALSE}
  ...
  for(sp in spIds)
  {    
    db <- oob_growth[species_id == sp, .(growth, dbh0, mean_temp_period_3_lag, tot_annual_pp_lag, canopyDistance, plot_id_seq)]
    ...
    # sample X random values from posterior dist
      ...
      for(i in 1:sampleN)
      {
        ...
        {
          # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
          # get growth rate with individual variation
          db[, g_var_pred := growth_model3(param = parsI, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = TRUE)]
        }else{
            # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
          # get growth rate with individual variation
