---
 ...
 subtitle: "Test v19.0 - Von Bertalanffy model - r in function of `BA_comp`, `bio_01_mean`, and `bio_12_mean`. Using previous measure as S_t instead of using the first measurement of the individual."
 author: "Will Vieira"
...
treeData[, .(species_id, dbh, deltaTime, BA_comp, bio_01_mean, bio_12_mean, sampled2)] |>
  mutate(sample = sampled2) |>
  pivot_longer(
    cols = c(dbh, deltaTime, BA_comp, bio_01_mean, bio_12_mean),
    names_to = 'var'
  ) |>
...
```{r size_BA_pred,echo=FALSE,fig.height = 8, fig.width = 10}
post |>
...
post |>
  filter(par %in% c('r', 'optimal_temp', 'sigma_temp')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  ...
  mutate(
    tempEffect = (-1/sigma_temp^2) * (bio_01_mean - optimal_temp)^2,
...
post |>
  filter(par %in% c('r', 'optimal_prec', 'sigma_prec')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  group_by(species_id) |>
  expand_grid(bio_12_mean = seq(300, 2700, 80)) |>
  mutate(
    precEffect = (-1/sigma_prec^2) * (bio_12_mean - optimal_prec)^2,
...
```{r BApred,echo=FALSE, warning=FALSE, message=FALSE,fig.height = 4.5, fig.width = 10}
...
post_mean <- post |>
  filter(
    par %in% c('Lmax', 'Beta', 'optimal_temp', 'sigma_temp', 'optimal_prec', 'sigma_prec')
  ) |>
...
for(Sp in spIds) 
{
  ...
  # add parameters
  train_sp[,
    c('Beta', 'Lmax', 'optimal_temp', 'sigma_temp', 'optimal_prec', 'sigma_prec') := post_mean[
                      species_id == Sp,
                      .(Beta, Lmax, optimal_temp, sigma_temp, optimal_prec, sigma_prec)
                    ]
  ]
  ...
  train_sp[,
    r_plotBaTemp := exp(
      log(r_plot) +
      BA_comp * Beta +
      (-1/sigma_temp^2) * (bio_01_mean - optimal_temp)^2 +
      (-1/sigma_prec^2) * (bio_12_mean - optimal_prec)^2
    )
  ]
  ...
  p1 <- train_sp |>
      ...
      ggplot(aes(x = BA_comp, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            yLim[2] * exp(x * post_mean |> filter(species_id == Sp) |> pull(Beta)),
          color = 1,
          linetype = 'dashed'
        )
  ...
  p2 <- train_sp |>
      ...
      ggplot(aes(x = bio_01_mean, y = value, color = sim)) +
        ...
        ylim(yLim) +
        geom_function(
          fun = function(x) yLim[2] * exp(
            (-1/(post_mean |> filter(species_id == Sp) |> pull(sigma_temp))^2) *
  ...
  p3 <- train_sp |>
      ...
      ggplot(aes(x = bio_12_mean, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x) yLim[2] * exp(
            (-1/(post_mean |> filter(species_id == Sp) |> pull(sigma_prec))^2) *
  ...
  print(
    ggpubr::ggarrange(
      p1, p2, p3,
      ncol = 3, legend = 'top', common.legend = TRUE)
  )
}
```
...
for(Sp in spIds)
{
  ...
  val_sp[
    ...
    `:=`(
      ...
      sigma_temp = i.sigma_temp,
      optimal_prec = i.optimal_prec,
      sigma_prec = i.sigma_prec
    ),
    on = 'iter'
  ]
