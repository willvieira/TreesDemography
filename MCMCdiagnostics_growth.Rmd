---
 ...
 subtitle: "Test v6.0 - Random effect on the baseline growth rate (pdg)"
 author: "Will Vieira"
...
# Load simulation variables
  ...
  sim1name <- 'Fixed effect'
  sim2name <- 'Random effect'
...
growth <- growth_dt[, .(ID_PE, sp_code2, growth, dbh0, dbh1, deltaYear, value5_bio60_01, value5_bio60_12, BA)]

# transform dt
growth <- tidyr::gather(data = growth,
            growth, dbh0, value5_bio60_01, value5_bio60_12, BA,
            key = 'var', value = 'value')
```
...
# Plot of size, temperature, precipitation and competition effect
  ...
  # define functions of the model
    ...
    compEffect = function(xBA, sigma_C) {
      return( exp(-((xBA^2)/2 * sigma_C^2)) )
    }
 ...
 # define x axis
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(BAx, compEffect(BAx, sigma_C), xlim = c(0, 80),
      ylim = c(0, 1), type = 'l', xlab = 'Canopy distance', ylab = 'Competition effect',
      col = spCols[sp],
      yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)
      points(BAx, compEffect(BAx, sigma_C), type = 'l', col = spCols[sp])
    }
...
# Plot predictions
  ...
  growth_model <- function(param, sim, newdata, indVar = FALSE)
  {
    ...
    mu_d <- param["pdg"] *
            exp(-((newdata$BA^2)/2*param["sigma_C"]^2)) *
  ...
  plot_prediction <- function(sim, sp, model, data) {
    ...
    for(indVar in c(FALSE, TRUE))
    {
      ...
      if(!indVar)
      {
        growth_meanT <- growth_model(param = param_mean, newdata = newdataT)
        growth_ci10T <- growth_model(param = param_ci10, newdata = newdataT)
        growth_ci90T <- growth_model(param = param_ci90, newdata = newdataT)
      }else {
        DF <- growth_model(param = param_mean, newdata = newdataT, indVar = TRUE)
      ...
      plot(growth ~ TP, growth_ci90T, type = "n", ylim = c(0, 5),
      ...
      plot(growth ~ PP, growth_ci90P, type = "n", ylim = c(0, 6),
...
```{r organize data by sp for OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  for(sp in spIds)
  {
    ...
    toSub <- readRDS(paste0('output/', simName, '/', sp, '/sampleInfo/toSub.RDS'))
    db[toSub, on = .(ID_PE), plot_id := i.plot_id]
...
```{r calculate OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  # growth model
  growth_model2 <- function(param, sim, D, TP, PP, BA, plot_id, indVar = FALSE)
  {
  ...
  for(sp in spIds)
  {
    for(sim in simulations)
    {
      ...
      if(sim == 'sim1')
      {
        ...
        oob_list[[sp]][, Gdet_mean_sim1 := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci10_sim1 := growth_model2(param = param_ci10, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim1 := growth_model2(param = param_ci90, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
        ...
        # sample parameters
        params <- lapply(posterior, sample, sampleN)
        for(i in 1:sampleN)
        {
          summ_mat[, i] <- oob_list[[sp]][, growth_model2(unlist(lapply(params, `[[`, i)), sim = sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = TRUE)]
        ...
        oob_list[[sp]][, Gvar_ci90_sim1 := apply(summ_mat, 1, quantile, probs = 0.9, na.rm = TRUE)]
          ...
          # get deterministic growth rate
        oob_list[[sp]][, Gdet_mean_sim2 := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci10_sim2 := growth_model2(param = param_ci10, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim2 := growth_model2(param = param_ci90, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
...
```{r calculate R2 from random forest and LMM,echo=FALSE,warning=FALSE}
  ...
  {
      # data from species
      db <- growth_dt[sp_code2 == sp, setdiff(names(growth_dt), varsToRm), with = FALSE]
      ...
      lmm_sp <- lmer(growth ~ 1 + (1 | ID_PE) +
                              (1 | year0) + (1 | ID_PE:year0) +
                              (BA + dbh0 + I(dbh0^2)) * (value5_bio60_01 + I(value5_bio60_01^2) +
                                  value5_bio60_12 + I(value5_bio60_12^2)), data = db_training, REML = TRUE)
...
```{r plot OOB predictions,echo=FALSE,fig.height = 8, fig.width = 12}
  ...
  reg_dt <- data.table(sp_code2 = rep(spIds, (length(simulations) + 2)), sim = character(n), R2 = numeric(n), Slope = numeric(n))
  ...
  for(sp in spIds)
  {
    ...
    rf_coeffSp <- rf_coeff[sp_code2 == sp, ]

    # add to reg table
    reg_dt[sp_code2 %in% sp, `:=`(sim = c(simulations, 'randomForest', 'LMM'),
    ...
    # legend
    leg <- c('1:1 line',
              latex2exp::TeX(paste0('LMM: R^2 = ', round(reg_dt[sp_code2 == sp & sim == 'LMM', R2], 2), '; slope = ', round(reg_dt[sp_code2 == sp & sim == 'LMM', Slope], 2))),
              latex2exp::TeX(paste0('randomForest: R^2 = ', round(reg_dt[sp_code2 == sp & sim == 'randomForest', R2], 2), '; slope = ', round(reg_dt[sp_code2 == sp & sim == 'randomForest', Slope], 2))),
              latex2exp::TeX(paste0('Bayesian sim1: R^2 = ', round(reg_dt[sp_code2 == sp & sim == 'sim1', R2], 2), '; slope = ', round(reg_dt[sp_code2 == sp & sim == 'sim1', Slope], 2))),
              latex2exp::TeX(paste0('Bayesian sim2: R^2 = ', round(reg_dt[sp_code2 == sp & sim == 'sim2', R2], 2), '; slope = ', round(reg_dt[sp_code2 == sp & sim == 'sim2', Slope], 2))))
    
    for(sim in simulations)
    {
      # sample 10% of data
      dbAll <- oob_growth[sp_code2 == sp, c('growth', grep(sim, names(oob_growth), value = TRUE)), with = FALSE]
      ...
      abline(rf_coeffSp[sp_code2 == sp]$Int_rf, rf_coeffSp[sp_code2 == sp]$Slope_rf, lty = 3)
      abline(rf_coeffSp[sp_code2 == sp]$Int_lmm, rf_coeffSp[sp_code2 == sp]$Slope_lmm, lty = 4)
...
```{r calculate MSE, echo = FALSE}
  ...
  # growth model
  growth_model3 <- function(param, sim, D, TP, PP, BA, plot_id, indVar = FALSE)
  {
    lgPart <- (log(D/param["Phi_opt"])/param["sigmaPhi_opt"])^2
  ...
  for(sp in spIds)
  {
    ...
    db <- oob_growth[sp_code2 == sp, .(growth, dbh0, value5_bio60_01, value5_bio60_12, BA, plot_id)]
    ...
    for(sim in simulations)
    {
      ...
      for(i in 1:sampleN)
      {
        ...
        {
          # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
          # get growth rate with individual variation
          db[, g_var_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = TRUE)]
        }else{
            # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, plot_id = plot_id, indVar = FALSE)]
          # get growth rate with individual variation
...
```{r plot MSE,echo=FALSE,fig.height = 6, fig.width = 8, warning=FALSE, message=FALSE}
  ...
  rf_coeff_lg <- rf_coeff %>%
                  pivot_longer(!sp_code2, names_to = 'Method', values_to = 'value') %>%
                  mutate(Method = plyr::revalue(Method, c('MSE_rf' = 'Random forest', 'MSE_lmm' = 'LMM')))

  # For deterministic growth
  ggplot(data = MSE_dt, aes(MSE_det, sp, fill = sim, colour = sim)) + 
    geom_density_ridges(alpha = 0.4, scale= 1.8) +
    geom_segment(data = subset(rf_coeff_lg, Method == 'Random forest'), aes(linetype = `Method`, x = value, xend = value, y = length(sp_code2):1,
                                      yend = (length(sp_code2):1) + .8),
               color = "black", alpha = 0.6) +
    geom_segment(data = subset(rf_coeff_lg, Method == 'LMM'), aes(linetype = `Method`, x = value, xend = value, y = length(sp_code2):1,
                                      yend = (length(sp_code2):1) + .8),
               color = "black", alpha = 0.6) +
    xlim(0.5, 7.6) +
    theme_classic() +
...
```{r plot Rsquared,echo=FALSE,fig.height = 6, fig.width = 8, warning=FALSE, message=FALSE}
  ...
  ggplot(data = MSE_dt, aes(r2, sp, fill = sim, colour = sim)) + 
    geom_density_ridges(alpha = 0.4, scale= 1.8) +
    geom_segment(data = subset(rf_coeff_r2, Method == 'Random forest'), aes(linetype = `Method`, x = value, xend = value, y = length(sp_code2):1,
                                      yend = (length(sp_code2):1) + .8),
               color = "black", alpha = 0.6) +
    geom_segment(data = subset(rf_coeff_r2, Method == 'LMM'), aes(linetype = `Method`, x = value, xend = value, y = length(sp_code2):1,
                                      yend = (length(sp_code2):1) + .8),
               color = "black", alpha = 0.6) +
    xlim(0, 0.64) +
    theme_classic() +
