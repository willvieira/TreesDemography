...
```{r calculate MSE, echo = FALSE}
  ...
  MSE_dt <- setDT(data.frame(sp = character(), sim = integer(), MSE_det = logical(), MSE_var = numeric()))
  sampleN <- 500
  ...
  for(sp in spIds)
  {
    ...
    for(sim in simulations)
    {
      ...
      MSE_det <- MSE_var <- numeric(sampleN)
      for(i in 1:sampleN)
      {
      ...
      # update main dt
      MSE_dt <- rbind(MSE_dt, data.frame(sp = rep(sp, sampleN), sim = rep(sim, sampleN), MSE_det = MSE_det, MSE_var = MSE_var))
    }
  }
```
...
```{r plot MSE,echo=FALSE,fig.height = 6, fig.width = 8, warning=FALSE, message=FALSE}
  ...
  MSE <- readRDS('data/quebec/randomForest_growth.RDS')
  MSE$RandomForest <- plyr::revalue(MSE$model, c('MSE1' = 'sim1', 'MSE2' = 'sim2', 'MSE_full' = 'sim_full'))

  # For deterministic growth
  ggplot(data = MSE_dt, aes(MSE_det, sp, fill = sim, colour = sim)) + 
    geom_density_ridges(alpha = 0.4, scale= 1.8) +
    geom_segment(data = subset(MSE, RandomForest == 'sim1'), aes(linetype = `RandomForest`, x = value, xend = value, y = length(sp):1,
                                      yend = (length(sp):1) + .8),
               color = "red", alpha = 0.6) +
    geom_segment(data = subset(MSE, RandomForest == 'sim2'), aes(linetype = `RandomForest`, x = value, xend = value, y = length(sp):1,
                                  yend = (length(sp):1) + .8),
            color = "#4aaab0", alpha = 0.6) +
    geom_segment(data = subset(MSE, RandomForest == 'sim_full'), aes(linetype = `RandomForest`, x = value, xend = value, y = length(sp):1,
                                  yend = (length(sp):1) + .8),
