---
 ...
 subtitle: "Test v3.0 - Test two different functions for competition (basal area)"
 author: "Will Vieira"
 date: "`r paste('Last updated on', format(Sys.time(), '%d %B, %Y'))`"
---


## Compare the convergence of the model using two different functions for the competition effect using Basal area:

- **sim1**: Linear function with intercept fixed at 1 and slope varying [-0.0001, 0.015]
- **sim2**: Modified half-normal function with sigma varying [0, 1]
...
growth_dt <- growth_dt[, .(sp_code2, growth, dbh0, value5_bio60_01, value5_bio60_12, BA)]
...
# Plot of size, temperature, precipitation and competition effect
  ...
  # define functions of the model
    ...
    compEffect = function(xBA, beta, sigma_C, sim) {
      if(sim == 'sim1') {
        return( beta * xBA + 1 )
      }else{
  ...
  for(sim in simulations) {
    for(sp in 1:length(spIds)) {
      ...
      if(sp == 1) plot(BAx, compEffect(BAx, beta, sigma_C, sim), xlim = c(0, 80),
      ylim = c(0, 1), type = 'l', xlab = 'Canopy distance', ylab = 'Competition effect',
      col = spCols[sp],
      yaxt = ifelse(sim == 'sim1', "s", "n"))
      if(sim == 'sim2') axis(2, labels = FALSE)

      points(BAx, compEffect(BAx, beta, sigma_C, sim), type = 'l', col = spCols[sp])
    }
...
# Plot predictions
  ...
  growth_model <- function(param, sim, newdata, indVar = FALSE)
  {
    ...
    if(sim == 'sim1') {
      competition <- newdata$BA * param["beta"] + 1
    }else {
      competition <- exp(-((newdata$BA^2)/2*param["sigma_C"]^2))
    }
    ...
    # TODO FIX IT - model 1 was formulated wrong
    mu_d[mu_d < 0] = 0.000001
...
```{r calculate OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  # growth model
  growth_model2 <- function(param, sim, D, TP, PP, BA, indVar = FALSE)
  {
    ...
    # competition function depending on simulation
    if(sim == 'sim1') {
      competition <- param["beta"] * BA + 1
    }else {
      competition <- exp(-((BA^2)/2 * param["sigma_C"]^2))
    }
  ...
  for(sp in spIds)
  {
    for(sim in simulations)
    {
      ...
      if(sim == 'sim1')
      {
        ...
        oob_list[[sp]][, Gdet_mean_sim1 := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci10_sim1 := growth_model2(param = param_ci10, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim1 := growth_model2(param = param_ci90, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]

        # get growth rate with individual variation
        oob_list[[sp]][, c('Gvar_mean_sim1', 'Gvar_ci10_sim1', 'Gvar_ci90_sim1') := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = TRUE)]
      }else{
          # get deterministic growth rate
        oob_list[[sp]][, Gdet_mean_sim2 := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci10_sim2 := growth_model2(param = param_ci10, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]
        oob_list[[sp]][, Gdet_ci90_sim2 := growth_model2(param = param_ci90, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]

        # get growth rate with individual variation
        oob_list[[sp]][, c('Gvar_mean_sim2', 'Gvar_ci10_sim2', 'Gvar_ci90_sim2') := growth_model2(param = param_mean, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = TRUE)]
      }
    }
  }
...
```{r plot OOB predictions,echo=FALSE,fig.height = 8, fig.width = 12}
  ...
  for(sp in spIds)
  {
    ...
    for(sim in simulations)
    {
      ...
      abline(lm(get(paste0('Gdet_mean_', sim)) ~ growth, data = dbAll), lty = 2)
      abline(predCurveRF[[sp]][3, 'Intercept'], predCurveRF[[sp]][3, 'Slope'], lty = 3)
      if(sim == 'sim1') legend('topleft', legend = c('1:1 line', 'lm() from Bayesian', 'lm() from randomForest'), lty = c(1, 2, 3), cex = 3, bty = 'n')
      ...
      abline(lm(get(paste0('Gvar_mean_', sim)) ~ growth, data = dbAll), lty = 2)
      abline(predCurveRF[[sp]][3, 'Intercept'], predCurveRF[[sp]][3, 'Slope'], lty = 3)
...
```{r calculate MSE, echo = FALSE}
  ...
  # growth model
  growth_model3 <- function(param, sim, D, TP, PP, BA, indVar = FALSE)
  {
  ...
  for(sp in spIds)
  {
    ...
    db <- oob_growth[sp_code2 == sp, .(growth, dbh0, value5_bio60_01, value5_bio60_12, BA)]
    ...
    for(sim in simulations)
    {
      ...
      for(i in 1:sampleN)
      {
        ...
        {
          # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]
          # get growth rate with individual variation
          db[, g_var_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = TRUE)]
        }else{
            # get deterministic growth rate
          db[, g_det_pred := growth_model3(param = parsI, sim, D = dbh0, TP = value5_bio60_01, PP = value5_bio60_12, BA = BA, indVar = FALSE)]
          # get growth rate with individual variation
