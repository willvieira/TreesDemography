...
growth_model <- function(sp, sim, pars, D, TP, PP, CD, indVar = FALSE)
{
  ...
  for(i in c('mean', 'ci10', 'ci90'))
  {
    ...
    if(sim == 'sim1') {
      compEffect <- (param['Lo'] + ((1 - param['Lo']) / (1 + exp(-0.45 * (CD - param['mid'])))))
    }else{
      compEffect <- (param['Lo'] + ((1 - param['Lo']) / (1 + exp(-param['beta'] * (CD)))))
    }
    mu_d <- param["pdg"] *
            compEffect *
...
growth_dt[, sizec := cut(dbh0, seq(min(dbh0), max(dbh0), length.out = 8)), by = species_id]
growth_dt[, tempc := cut(bio_01, seq(min(bio_01), max(bio_01), length.out = 8)), by = species_id]
growth_dt[, precc := cut(bio_12, seq(min(bio_12), max(bio_12), length.out = 8)), by = species_id]
growth_dt[, compc := cut(canopyDistance, seq(min(canopyDistance), max(canopyDistance), length.out = 8)), by = species_id]
...
for(sp in spIds)
{
  ...
  {  
    ...
    sp_dt[, plot(dbh0, growth, pch = 19, cex = .6, col = rgb(0, 0, 0, 0.4), ylim = c(0, quantile(growth, 0.99)), xlab = 'Size (mm)', ylab = '', yaxt = ifelse(sim == 'sim1', "s", "n"))]
    polygon(
      c(x, rev(x)),
      c(growth$ci10, rev(growth$ci90)),
      col = alpha("deepskyblue4", 0.3), border = NA
    )
    points(x, growth$mean, type = 'l', lwd = 2, col = 'deepskyblue4')
    ...
    sp_dt[, plot(bio_01, growth, pch = 19, cex = .6, col = rgb(0, 0, 0, 0.4), ylim = c(0, quantile(growth, 0.99)), xlab = 'Mean annual temperature (°C)', ylab = '', yaxt = ifelse(sim == 'sim1', "s", "n"))]
    polygon(
      c(x, rev(x)),
      c(growth$ci10, rev(growth$ci90)),
      col = alpha("deepskyblue4", 0.1), border = NA
    )
    points(x, growth$mean, type = 'l', lwd = 2, col = 'deepskyblue4')
    ...
    sp_dt[, plot(bio_12, growth, pch = 19, cex = .6, col = rgb(0, 0, 0, 0.4), ylim = c(0, quantile(growth, 0.99)), xlab = 'Total annual precipitation (mm)', ylab = '', yaxt = ifelse(sim == 'sim1', "s", "n"))]
    polygon(
      c(x, rev(x)),
      c(growth$ci10, rev(growth$ci90)),
      col = alpha("deepskyblue4", 0.1), border = NA
    ...
    sp_dt[, plot(canopyDistance, growth, pch = 19, cex = .6, col = rgb(0, 0, 0, 0.4), ylim = c(0, quantile(growth, 0.99)), xlab = 'Canopy distance (m)', ylab = '', yaxt = ifelse(sim == 'sim1', "s", "n"))]
    polygon(
      c(x, rev(x)),
      c(growth$ci10, rev(growth$ci90)),
      col = alpha("deepskyblue4", 0.1), border = NA
...
```{r plot OOB predictions,echo=FALSE,fig.height = 8, fig.width = 12}
  ...
  for(sp in spIds)
  {
    ...
    for(sim in simulations)
    {
      ...
      # plot growth with individual variation
