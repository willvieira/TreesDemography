---
 ...
 subtitle: "Test v17.1 - Von Bertalanffy model - r with plot and individual random effects on `r`"
 author: "Will Vieira"
...
print(
  treeEffect |>
    ggplot(aes(x = dbh, m_rReal, color = BA_comp)) +
        geom_point(size = 0.6) +
        facet_wrap(~species_id, scales = 'free') +
        geom_hline(data = r_pop, aes(yintercept = r), color = 'red') +
        xlab('dbh (mm)') +
        ylab('Individual random effect') +
        theme_minimal()
)

print(
  treeEffect |>
    ggplot(aes(x = BA_comp, m_rReal, color = dbh)) +
        geom_point(size = 0.6) +
        facet_wrap(~species_id, scales = 'free') +
        geom_hline(data = r_pop, aes(yintercept = r), color = 'red') +
        xlab('Plot BA of larger individuals') +
        ylab('Individual random effect') +
        theme_minimal()
)
...
print(
  treeEffect |>
    ggplot(aes(x = bio_01_mean, m_rReal, color = dbh)) +
        geom_point(size = 0.6) +
        facet_wrap(~species_id, scales = 'free') +
        geom_hline(data = r_pop, aes(yintercept = r), color = 'red') +
        xlab('Annual mean temperature') +
...
# save plot effect ot be used against future models
treeEffect |>
  select(species_id, tree_id, m_rReal) |>
  saveRDS(
    file.path(
        'output', 
        simName,
...
## Growth in function of Basal area competition (But basal area is not in the model!)
...
for(Sp in spIds) 
{
  ...
  # add parameters
  train_sp[, c('Lmax') := post_mean[species_id == Sp, .(Lmax)]]
  train_sp[
    plotEffect |>
  ...
  # compute random effects on r
  train_sp[, r_plotTree := exp(log(r_plot) + log(r_tree) - log(r_pop))]
  
  # predict size
  train_sp[,
    pred_size := dbh0 *
                exp(-r_plotTree * deltaTime) +
                Lmax * (1 -exp(-r_plotTree * deltaTime))
  ]
...
for(Sp in spIds)
{
  ...
  # as validation and training were selected at the level of tree_id
  # there is no same individual shared between training and validation
  val_sp[,
    rTree_log := rnorm(.N, 0, sigma_tree)
  ]
  ...
  val_sp[,
    r_plot := exp(r + rPlot_log + rTree_log)
  ]
