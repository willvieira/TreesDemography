---
 ...
 subtitle: "Test v14.0 - Von Bertalanffy model - r in function of BA_comp"
 author: "Will Vieira"
...
# compute growth
train[, time0 := shift(time, type = 'lag'), by = tree_id]
train[, dbh0 := shift(dbh, type = 'lag'), by = tree_id]
train[, growth := (dbh - dbh0)/(time - time0)]
...
for(sp_seq in seq(1, 17, 2)) 
{
  ...
  for(i in 0:1)
  {
    ...
    train_sp[, r_plotBA := exp(log(r_plot) + BA_comp^2 * 1/2 * -beta^2)]
    
    # predict size
    train_sp[,
      pred_size := start_size *
                  exp(-r_plotBA * time) +
                  Lmax * (1 -exp(-r_plotBA * time))
    ]

    # compute growth from predicted size
    train_sp[, pred_size0 := shift(pred_size, type = 'lag'), by = tree_id]
    train_sp[, pred_growth := (pred_size - pred_size0)/(time - time0)]
...
for(Sp in spIds)
{
  ...
  val_sp[,
    r_plot := exp(r + rPlot_log + BA_comp^2 * 1/2 * -beta^2)
  ]
  ...
  pred_growth_matrix <- val_sp[time > 0 & !is.na(obs_growth) & !is.na(pred_growth), .(pred_growth_random, iter2, obsID)] |>
      pivot_wider(names_from = obsID, values_from = pred_growth_random) |>
      select(-iter2) |>
      as.matrix()

  p4 <- ppc_dens_overlay(
      val_sp[iter2 == 1 & time > 0 & !is.na(obs_growth), obs_growth],
      pred_growth_matrix
  ) +
...
map_dfr( 
  ...
  select(iter2, species_id, icc, R2reg_growth) |>
  group_by(species_id) |>
  summarise(
    icc_mean = mean(icc),
    icc_min = quantile(icc, probs = .05),
    icc_max = quantile(icc, probs = .95),
    r2_mean = mean(R2reg_growth),
    r2_min = quantile(R2reg_growth, probs = .05),
    r2_max = quantile(R2reg_growth, probs = .95)
  ) |>
