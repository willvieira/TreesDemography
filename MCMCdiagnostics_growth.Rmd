...
```{r calculate OOB predictions,echo=FALSE,fig.height = 12, fig.width = 12}
  ...
  # growth model
  growth_model2 <- function(param, sim, D, TP, PP, C, plot_id, indVar = FALSE)
  {
  ...
  for(sp in spIds)
  {
    for(sim in simulations)
    {
      ...
      if(sim == 'sim1')
      {
        ...
        oob_list[[sp]][, Gdet_mean_sim1 := growth_model2(param = param_mean, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
        ...
        oob_list[[sp]][, Gvar_ci90_sim1 := apply(summ_mat, 1, quantile, probs = 0.9, na.rm = TRUE)]
          ...
          # get deterministic growth rate
        oob_list[[sp]][, Gdet_mean_sim2 := growth_model2(param = param_mean, sim, D = dbh0, TP = mean_temp_period_3_lag, PP = tot_annual_pp_lag, C = canopyDistance, plot_id = plot_id_seq, indVar = FALSE)]
