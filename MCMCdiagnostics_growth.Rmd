...
growth_dt[, compsim1c := cut(canopyDistance, seq(min(canopyDistance), max(canopyDistance), length.out = 8)), by = species_id]
growth_dt[, compsim2c := cut(BA, seq(min(BA), max(BA), length.out = 8)), by = species_id]
...
for(sp in spIds)
{
  ...
  {  
    ...
    if(sim == 'sim1') {
      sp_dt[, plot(canopyDistance, growth, pch = 19, cex = .8, col = rgb(0, 0, 0, 0.4), ylim = c(0, quantile(growth, 0.99)), xlab = 'Canopy distance (m)', ylab = '', yaxt = ifelse(sim == 'sim1', "s", "n"))]
    }else{
      sp_dt[, plot(BA, growth, pch = 19, cex = .8, col = rgb(0, 0, 0, 0.4), ylim = c(0, quantile(growth, 0.99)), xlab = 'Basal area (m2/ha)', ylab = '', yaxt = ifelse(sim == 'sim1', "s", "n"))]
    }
