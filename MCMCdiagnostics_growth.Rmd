---
 ...
 subtitle: "Test v20.0 - Vanderwel with basal area, temperature, and precipitation effects"
 author: "Will Vieira"
 date: "`r paste('Last updated on', format(Sys.time(), '%d %B, %Y'))`"
---


## Diagnostics for the model adapted from Vandewel 2014
      ...
      ggarrange(
          dt_toPlot |>
              ggplot(aes(y = plot_id, x = r_real, color = bio_01_mean)) +
                ...
                xlab('pdg') + ylab('plot_id (ordered by latitude)') +
                theme(
                  legend.position = 'top',
...
```{r compare_rReal, eval=FALSE, echo = FALSE, warning=FALSE, message=FALSE, fig.height = 8, fig.width = 12}
...
```{r organize data, echo = FALSE, warning=FALSE, message=FALSE, fig.height = 10, fig.width = 12}
...
treeData[, .(species_id, dbh, BA_comp, bio_01_mean, bio_12_mean, sampled2)] |>
  mutate(sample = sampled2) |>
  pivot_longer(
    cols = c(dbh, BA_comp, bio_01_mean, bio_12_mean),
    names_to = 'var'
  ) |>
...
post |>
  filter(par %in% c('Phi_opt', 'sigmaPhi_opt')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  group_by(species_id) |>
  expand_grid(dbh = seq(100, 1000, 20)) |>
  mutate(
    size_effect = exp(-(log(dbh/Phi_opt)^2)/sigmaPhi_opt^2)
  ) |>
  ggplot(aes(x = dbh, y = size_effect, color = species_id)) +
        stat_lineribbon(.width = 0.95, alpha = .8) +
        scale_fill_brewer() +
        xlab('DBH size (mmm)') + ylab('Size effect') +
        theme_minimal()
```

## Basal area effect

```{r size_BA_pred,echo=FALSE,fig.height = 6.5, fig.width = 8}
post |>
  filter(par %in% 'sigma_C') |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  ...
  mutate(
    BAeffect = exp(-BA_comp^2/2 * sigma_C^2)
  ) |>
  ggplot(aes(x = BA_comp, y = BAeffect, color = species_id)) +
    stat_lineribbon(.width = 0.95, alpha = .8) +
    scale_fill_brewer() +
...
post |>
  filter(par %in% c('T_opt', 'sigmaT_opt')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  group_by(species_id) |>
  expand_grid(bio_01_mean = seq(-6, 25, 1)) |>
  mutate(
    tempEffect = exp(-0.5 * (bio_01_mean - T_opt)^2/sigmaT_opt^2)
  ) |>
  ggplot(aes(x = bio_01_mean, y = tempEffect, color = species_id)) +
    stat_lineribbon(.width = 0.9, alpha = .8) +
    scale_fill_brewer() +
    xlab('Mean annual temperature (MAT)') +
    ylab('MAT effect') +
    theme_minimal()
```
...
post |>
  filter(par %in% c('P_opt', 'sigmaP_opt')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  group_by(species_id) |>
  expand_grid(bio_12_mean = seq(400, 2700, 50)) |>
  mutate(
    precEffect = exp(-0.5 * (bio_12_mean - P_opt)^2/sigmaP_opt^2)
  ) |>
  ggplot(aes(x = bio_12_mean, y = precEffect, color = species_id)) +
    stat_lineribbon(.width = 0.9, alpha = .8) +
    scale_fill_brewer() +
    xlab('Mean annual temperature (MAT)') +
    ylab('MAT effect') +
...
## Growth in function of Basal area and mean annual temperature

```{r BApred,echo=FALSE, warning=FALSE, message=FALSE,fig.height = 9, fig.width = 9}
...
# prepare parameters
post_mean <- post |>
  filter(par %in% c('pdg', 'T_opt', 'sigmaT_opt', 'P_opt', 'sigmaP_opt', 'sigma_C', 'Phi_opt', 'sigmaPhi_opt')) |>
...
for(Sp in spIds) 
{
  ...
  # add parameters
  train_sp[, c('pdg', 'T_opt', 'sigmaT_opt', 'P_opt', 'sigmaP_opt', 'sigma_C', 'Phi_opt', 'sigmaPhi_opt') :=
    post_mean[species_id == Sp, .(pdg, T_opt, sigmaT_opt, P_opt, sigmaP_opt, sigma_C, Phi_opt, sigmaPhi_opt)]]
  train_sp[
    plotEffect |>
  ...
  train_sp[,
    pred_growth :=
		  pdg_plot
		  *
		  exp(-BA_comp^2/2 * sigma_C^2)
		  *
		  (0.0001 + exp(-0.5 * (bio_01_mean - T_opt)^2/sigmaT_opt^2)
		  *
		  exp(-0.5 * (bio_12_mean - P_opt)^2/sigmaP_opt^2))
		  *
		  exp(-log(dbh/Phi_opt)^2/sigmaPhi_opt^2)
  ]
  ...
  p1 <- train_sp |>
      ...
      select(dbh, obs_growth, pred_growth) |>
      pivot_longer(
        cols = contains('growth'),
        names_to = 'sim'
      ) |>
      ggplot(aes(x = dbh, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            Int * exp(-log(x/post_sp$Phi_opt)^2/post_sp$sigmaPhi_opt^2),
          color = 1,
          linetype = 'dashed'
        )
  ...
  p2 <- train_sp |>
      ...
      ggplot(aes(x = BA_comp, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            Int * exp(-x^2/2 * post_sp$sigma_C^2),
          color = 1,
          linetype = 'dashed'
        )
  ...
  p3 <- train_sp |>
      ...
      ggplot(aes(x = bio_01_mean, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            Int * exp(-0.5 * (x - post_sp$T_opt)^2/post_sp$sigmaT_opt^2),
          col = 1,
          linetype = 'dashed'
        )
  ...
  p4 <- train_sp |>
      ...
      ggplot(aes(x = bio_12_mean, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            Int * exp(-0.5 * (x - post_sp$P_opt)^2/post_sp$sigmaP_opt^2),
          col = 1,
          linetype = 'dashed'
        )
  ...
  print(
    ggpubr::ggarrange(
      p1, p2, p3, p4,
      ncol = 2, nrow = 2, legend = 'top', common.legend = TRUE)
  )
}
```
...
for(Sp in spIds)
{
  ...
  val_sp[
    ...
    `:=`(
      pdg = i.pdg,
      pdg_psd = i.pdg_psd,
      sigma_base = i.sigma_base,
      T_opt = i.T_opt,
      sigmaT_opt = i.sigmaT_opt,
      P_opt = i.P_opt,
      sigmaP_opt = i.sigmaP_opt,
      sigma_C = i.sigma_C,
      Phi_opt = i.Phi_opt,
      sigmaPhi_opt = i.sigmaPhi_opt
    ),
    on = 'iter'
  ]
  ...
  # compute pdg plot specific parameter
  val_sp[,
    pdg_plot := exp(pdg + pdgPlot_log)
  ]
  ...
  # compute observed growth
  val_sp[, obs_growth := (dbh - dbh0)/deltaTime]

  # simulate growth from model distribution (normal dist)
  val_sp[, pred_growth_random := rnorm(.N, pred_growth, sigma_base)]

  # calculate predicted future size
  val_sp[, pred_size := dbh0 + pred_growth * deltaTime]
  val_sp[, pred_size_random := dbh0 + pred_growth_random * deltaTime]
  ...
  R2gel <- val_sp |>
    group_by(iter2) |>
    summarise(var_growth = var(pred_growth, na.rm = TRUE)) |>
    bind_cols(iter = sampledIter) |>
    left_join(
      post |>
        filter(species_id == Sp & par == 'sigma_base') |>
        mutate(sigma_base = value) |>
        select(iter, sigma_base, species_id)
    ) |>
    mutate(R2gelman_growth = var_growth/(var_growth + sigma_base^2)) |>
    select(iter, iter2, species_id, R2gelman_growth) |>
    setDT()
  ...
  p1 <- val_sp[, .(m_obs = mean(dbh), m_pred = mean(pred_size)), by = obsID] |>
  ...
  p2 <- val_sp[, .(m_obs = mean(obs_growth), m_pred = mean(pred_growth)), by = obsID] |>
      ggplot(aes(m_obs, m_pred)) +
          geom_hex(bins = 100) +
          geom_abline(slope = 1, intercept = 0) +
          xlim(yLim) + ylim(yLim) +
  ...
  pred_size_matrix <- val_sp[!is.na(pred_size_random), .(pred_size_random, iter2, obsID)] |>
  ...
  pred_growth_matrix <- val_sp[!is.na(obs_growth) & !is.na(pred_growth), .(pred_growth_random, iter2, obsID)] |>
      pivot_wider(names_from = obsID, values_from = pred_growth_random) |>
      select(-iter2) |>
      as.matrix()

  p4 <- ppc_dens_overlay(
      val_sp[iter2 == 1 & !is.na(obs_growth), obs_growth],
      pred_growth_matrix
  ) +
...
map_dfr(
  ...
  ggplot(aes(x = R2gelman_growth, y = species_id, fill = sim)) +
      geom_density_ridges(alpha = 0.8, scale= 1.8, color = NA) +
