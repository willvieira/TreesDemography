...
post |>
  ...
  mutate(
    sim = factor(sim, levels = c('BAeffect', 'm'), labels = c('BA_adult effect on r parameter', 'm parameter'))
  ) |>
...
for(sp_seq in seq(1, 17, 2)) 
{
  ...
  for(i in 0:1)
  {
    ...
    tree_ids <- fec_sp[, .N, by = tree_id][N > 2, sample(tree_id, 1000)]
    fec_sp <- fec_sp[tree_id %in% tree_ids]
    # add parameters
    fec_sp[, c('beta', 'Lmax') := post_mean[species_id == Sp, .(beta, Lmax)]]
    fec_sp[
      plotEffect |>
    ...
    fec_sp[, r_plotBA := exp(log(r_plot) + BA_comp * beta)]
    ...
    fec_sp[, pred_size0 := shift(pred_size, type = 'lag'), by = tree_id]
    fec_sp[, pred_growth := (pred_size - pred_size0)/(time - time0)]
    plot_ls[[i+1]] <- fec_sp |>
