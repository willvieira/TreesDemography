---
 ...
 subtitle: "Test v7.0 - Ingrowth model following a negative binomial distribution accounting for ingrowth and mortality of new individuals with plot random effect and basal area efffects on ingrowth rate `m`"
 author: "Will Vieira"
...
# prepare parameters
post_mean <- post |>
  filter(par %in% c('mPop_log', 'p', 'beta', 'phi')) |>
...
for(sp_seq in seq(1, 17, 2)) 
{
  ...
  for(i in 0:1)
  {
    ...
    # add parameters
    fec_sp[,
      c('beta', 'mPop_log', 'p', 'phi') := post_mean[
        species_id == Sp,
        .(beta, mPop_log, p, phi)
      ]
    ]
    ...
    fec_sp[,
      pred_nbRecruit_random := rnbinom(.N, mu = pred_nbRecruit, size = phi)
    ]
  ...
  print(
    ggpubr::ggarrange(plotlist = plot_ls, ncol = 2, legend = 'bottom', common.legend = TRUE)
  )
}
```
...
for(Sp in spIds)
{
  ...
  val_sp[, lambda_random := rnbinom(.N, mu = lambda, size = phi)]
  ...
  R2gel <- val_sp |>
    ...
    left_join(
      post |>
        filter(species_id == Sp & par == 'phi') |>
        mutate(phi = value) |>
        select(iter, phi, species_id)
    ) |>
    mutate(
      varNegBinom = mean_lambda + (mean_lambda^2)/phi,
      R2gelman = var_lambda/(var_lambda + varNegBinom)
    ) |>
