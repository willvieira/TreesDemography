---
 ...
 subtitle: "Test v6.0 - Ingrowth model accounting for ingrowth and mortality of new individuals with species basal area efffects on ingrowth rate `m`, and total basal area effects on `p`."
 author: "Will Vieira"
...
```{r sizePred2,echo=FALSE,fig.height = 7.5, fig.width = 10}
# BA_adult_sp effect on `m`
post |>
  filter(par %in% c('mPop_log', 'beta_m')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
  group_by(species_id) |>
  expand_grid(BA_adult_sp = seq(0, 100, 5)) |>
  mutate(
    BAeffect = BA_adult_sp * beta_m,
...
post |>
  ...
  group_by(species_id) |>
  expand_grid(BA_adult = seq(0, 100, 5)) |>
  mutate(
    BAeffect = BA_adult^2 * 1/2 * -beta_p^2,
...
```{r BApred,echo=FALSE, warning=FALSE, message=FALSE,fig.height = 7, fig.width = 9}
# prepare parameters
post_mean <- post |>
  filter(par %in% c('mPop_log', 'p_log', 'beta_m', 'beta_p')) |>
...
for(Sp in spIds) 
{
  ...
  # add parameters
  fec_sp[,
    c('mPop_log', 'p_log', 'beta_m', 'beta_p') := post_mean[
      species_id == Sp,
      .(mPop_log, p_log, beta_m, beta_p)
    ]
  ]
  ...
  fec_sp[, m_plotBA := exp(log(m_mean) + BA_adult_sp * beta_m)]

  # compute BA_adult effect on p
  fec_sp[, p := exp(-exp(p_log)) + BA_adult^2 * 1/2 * -beta_p^2]
  ...
  print(
    fec_sp |>
      ...
      select(BA_adult, BA_adult_sp, obs_nbRecruit, pred_nbRecruit) |>
      pivot_longer(
