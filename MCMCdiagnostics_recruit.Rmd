---
 ...
 subtitle: "Test v5.0 - Ingrowth model accounting for ingrowth and mortality of new individuals with plot random effect and basal area efffects on ingrowth rate `m` with immigration parameter."
 author: "Will Vieira"
...
fec_dt[, .(species_id, nbRecruit, deltaYear_plot, BA_adult, relativeBA_adult_sp)] |>
  pivot_longer(
    cols = c(nbRecruit, deltaYear_plot, BA_adult, relativeBA_adult_sp),
    names_to = 'var'
  ) |>
  ggplot(aes(x = value, y = species_id)) +
...
post |>
  ...
  mutate(
    lambda = (exp(mPop_log) + m_imm) * 399 * (1 - p^deltaYear)/(1 - p)
  ) |>
...
post |>
  filter(par %in% c('mPop_log', 'beta', 'm_imm')) |>
  pivot_wider(
    names_from = par,
    values_from = value
  ) |>
...
# prepare parameters
post_mean <- post |>
  filter(par %in% c('mPop_log', 'p', 'beta', 'm_imm')) |>
...
for(sp_seq in seq(1, 17, 2)) 
{
  ...
  for(i in 0:1)
  {
    ...
    # add parameters
    fec_sp[,
      c('beta', 'mPop_log', 'p', 'm_imm') := post_mean[
        species_id == Sp,
        .(beta, mPop_log, p, m_imm)
      ]
    ]
    ...
    fec_sp[, m_plotBA := exp(log(m_mean) + BA_adult * -beta) * relativeBA_adult_sp + m_imm]
