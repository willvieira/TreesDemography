---
 ...
 subtitle: "Test v1.0 - allow fast transition to second stage"
 author: "Will Vieira"
...
# Load simulation variables
  ...
  sim1name <- 'No fast transition'
  sim2name <- 'Fast transition'
...
# Load rStan output and scaleInfo for all species_id
  ...
  for(sim in simulations) {
    for(sp in spIds) {
      assign(
        paste('fit', sim, sp, sep = '_'),
        readRDS(paste0('output/', simName, '/', 'out_', sp, '_', sim, '.RDS'))
      )
    }
  }
  ...
  for(sim in simulations)
    assign(paste0('pars_', sim), setdiff(get(ls(pattern = sim)[1])$parameters, 'Ntotal'))
...
```{r, echo = F, fig.height = 5, fig.width = 10}
  ...
  for(sim in simulations)
  {
    for(sp in spIds)
    {
      rhat_simSp <- get(paste('fit', sim, sp, sep = '_'))$Rhat
      rhat_pars <- unlist(rhat_simSp[names(rhat_simSp) %in% get(paste0('pars_', sim))])
...
# Trace and density plot of each parameter by species id
...
```{r get MCMC sampling data, echo = F}
  ...
  for(sim in simulations)
  {
    for(sp in spIds)
    {
      ...
      # get samples and filter the parameters of interest
      sim_samples <- do.call(rbind, simObj$samples) %>%
        as.data.frame() %>%
        select(!contains('Ntotal')) %>%
        pivot_longer(
          cols = everything(),
          names_to = 'pars', 
          values_to = 'value'
...
```{r plot mcmcSample by pars, echo = F, message=FALSE, fig.height = 6, fig.width = 9}

  for(par in unique(mcmcSampleDF$pars))
  {
    p <- mcmcSampleDF %>%
      filter(pars == par) %>%
      ggplot(aes(value, sp, fill = sim, color = sim)) +
...
pred_sim1 <- function(newdata, params, maxTime)
{
  ...
  for(Time in 2:dim(newdata_future)[2])
  {
    ...
    G_1 <- rpois(n = nSites, lambda = params$gamma[1] * subset(newdata_lg, year == Time-1 & state == 4)$pred + params$gamma[2])
    for(state in 2:4)
      assign(
        paste0('G_', state),
...
pred_sim2 <- function(newdata, params, maxTime)
{
  nSites <- dim(newdata)[1]
...
for(sp in spIds)
{
  ...
  # Load parameters for all simulations
  for(sim in simulations)
    assign(
      paste0('params_', sim), get(paste0('fit_', sim, '_', sp))$mean
    )
  ...
  {
    ## sim1
    preddata <- pred_sim1(newdata, get(paste0('fit_', simulations[1], '_', sp))$mean, maxTime)
    ...
    ## sim2
    preddata <- pred_sim2(newdata, get(paste0('fit_', simulations[2], '_', sp))$mean, maxTime)
