---
 ...
 subtitle: "Test v14.0 - Ingrowth model accounting for ingrowth and mortality of new individuals with species basal area and climate effects on ingrowth rate `m`, and total basal area effects on `p`."
 author: "Will Vieira"
...
plot_summary <- fec_dt[,  
  .( 
    ...
    BA_adult_sp = mean(BA_adult_sp, na.rm = TRUE),
...
for(Sp in spIds)
{
  ...
  plotEffect_ls[[Sp]] <- dt_toPlot |>
    ...
    select(
      ...
      BA_plot, BA_adult, BA_adult_sp, bio_01_mean, bio_12_mean
    )
...
sim_toCompare <- setdiff(simInfo$simCompare, 'intercept')

for(Sim in sim_toCompare)
{
...
fec_dt[, .(species_id, nbRecruit, deltaYear_plot, BA_adult, BA_adult_sp)] |>
  pivot_longer(
    cols = c(nbRecruit, deltaYear_plot, BA_adult, BA_adult_sp),
    names_to = 'var'
  ) |>
  ggplot(aes(x = value, y = species_id)) +
...
```{r sizePred_BA_sp,echo=FALSE,fig.height = 7.5, fig.width = 10}
post |>
  ...
  group_by(species_id) |>
  expand_grid(BA_adult_sp = seq(0, 60, 2.5)) |>
  mutate(
    BAeffect = (-1/sigma_BA^2) * (BA_adult_sp - optimal_BA)^2,
    m = exp(mPop_log + BAeffect)
  ) |>
  select(iter, species_id, BA_adult_sp, BAeffect, m) |>
  pivot_longer(
    cols = c('BAeffect', 'm'),
    names_to = 'sim'
  ) |>
  mutate(
    sim = factor(sim, levels = c('BAeffect', 'm'), labels = c('Conspecific basal area effect on m parameter', 'm parameter'))
  ) |>
  ggplot(aes(x = BA_adult_sp, y = value, color = species_id)) +
    stat_lineribbon(.width = 0.95, alpha = .8) +
...
```{r sizePred_BA,echo=FALSE,fig.height = 7.5, fig.width = 10}
post |>
  ...
  group_by(species_id) |>
  expand_grid(BA_adult = seq(0, 100, 3)) |>
  mutate(
    BAeffect = BA_adult * -beta_p,
    p = exp(-exp(p_log) + BAeffect)
  ) |>
  select(iter, species_id, BA_adult, BAeffect, p) |>
  pivot_longer(
    cols = c('BAeffect', 'p'),
    names_to = 'sim'
  ) |>
  mutate(
    sim = factor(sim, levels = c('BAeffect', 'p'), labels = c('BA_adult effect on p parameter', 'p parameter'))
  ) |>
...
```{r BApred,echo=FALSE, warning=FALSE, message=FALSE,fig.height = 7, fig.width = 10}
# prepare parameters
post_mean <- post |>
  filter(par %in% c('mPop_log', 'p_log', 'optimal_BA', 'sigma_BA', 'beta_p')) |>
...
for(Sp in spIds) 
{
  ...
  # add parameters
  fec_sp[,
    c('mPop_log', 'p_log', 'optimal_BA', 'sigma_BA', 'beta_p') := post_mean[
      species_id == Sp,
      .(mPop_log, p_log, optimal_BA, sigma_BA, beta_p)
    ]
  ]
  ...
  # compute BA_adult_sp effect on m
  fec_sp[, m_plotBA := exp(log(m_mean) + (-1/sigma_BA^2) * (BA_adult_sp - optimal_BA)^2)]

  # compute BA_adult effect on p
  fec_sp[, p := exp(-exp(p_log)) + BA_adult * -beta_p]
  ...
  yMax <- fec_sp[, quantile(nbRecruit, probs = .99)]
  ...
  p1 <- fec_sp |>
      ...
      mutate(
        obs_nbRecruit = nbRecruit + 0.08,
        pred_nbRecruit = pred_nbRecruit_random - 0.08
      ) |>
      select(BA_adult_sp, obs_nbRecruit, pred_nbRecruit) |>
      pivot_longer(
        cols = contains('Recruit'),
        names_to = 'sim'
      ) |>
      ggplot(aes(x = BA_adult_sp, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            exp(
              post_sp$mPop_log +
              (-1/post_sp$sigma_BA^2) * (x - post_sp$optimal_BA)^2
            ) *
            399 *
            (1 - exp(-exp(post_sp$p_log))^5) /
            (1 - exp(-exp(post_sp$p_log))),
          color = 1,
          linetype = 'dashed'
        )
  ...
  p2 <- fec_sp |>
      ...
      select(BA_adult, obs_nbRecruit, pred_nbRecruit) |>
      pivot_longer(
        cols = contains('Recruit'),
        names_to = 'sim'
      ) |>
      ggplot(aes(x = BA_adult, y = value, color = sim)) +
        ...
        geom_function(
          fun = function(x)
            ...
            399 *
            (1 - exp(-exp(post_sp$p_log) + x * -post_sp$beta_p)^5) /
            (1 - exp(-exp(post_sp$p_log) + x * -post_sp$beta_p)),
          color = 1,
          linetype = 'dashed'
        )
  ...
  print(
    ggpubr::ggarrange(
      p1, p2,
      ncol = 2, legend = 'top', common.legend = TRUE)
  )
...
for(Sp in spIds)
{
  ...
  val_sp <- val_sp[
    ...
    .(
      ...
      plot_size, BA_adult, BA_adult_sp
    )
  ]
