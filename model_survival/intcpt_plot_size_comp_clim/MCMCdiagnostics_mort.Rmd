---
 ...
 subtitle: "Model: intercept + plot random effects + size + competition + climate"
 author: "Will Vieira"
...
post |>
  ...
  mutate(
    psi_size = exp(psi + -(log(size/size_opt)/size_var)^2)
  ) ->
pred_data
...
pred_data |>
  ...
  geom_text_repel(
    data = pred_data |>
      ...
      mutate(
        max_x = max(size),
        max_y = mean(psi_size[size == max_x])
      ) |>
      slice_head(n = 1),
...
post |>
  ...
  mutate(
    psi_clim = exp(psi + -tau_temp * (clim - optimal_temp)^2)
  ) ->
pred_data
...
pred_data |>
  ...
  geom_text_repel(
    data = pred_data |>
      ...
      mutate(
        max_x = max(clim),
        max_y = mean(psi_clim[clim == max_x])
      ) |>
      slice_head(n = 1),
    mapping = aes(x = max_x, y = max_y, label = species_name),
    max.overlaps = Inf,
...
post |>
  ...
  expand_grid(clim = seq(0, 1, 0.05)) |>
  mutate(
    psi_clim = exp(psi + -tau_prec * (clim - optimal_prec)^2)
  ) ->
pred_data
...
# Compute model accuracy
  ...
  val_sp |>
    select(iter2, dbh0, mort, mort_random) |>
    mutate(
      sizec = cut(
        dbh0,
        seq(min(dbh0), max(dbh0), length.out = 10),
        include.lowest = TRUE
      )
    ) |>
  ...
  val_sp |>
    select(iter2, bio_01_mean_scl, mort, mort_random) |>
    filter(!is.na(bio_01_mean_scl)) |>
    mutate(
      varc = cut(
        bio_01_mean_scl,
        seq(min(bio_01_mean_scl), max(bio_01_mean_scl), length.out = 10),
        include.lowest = TRUE
      )
    ) |>
    group_by(varc, iter2) |>
  ...
  val_sp |>
    select(iter2, bio_12_mean_scl, mort, mort_random) |>
    filter(!is.na(bio_12_mean_scl)) |>
    mutate(
      varc = cut(
        bio_12_mean_scl,
        seq(min(bio_12_mean_scl), max(bio_12_mean_scl), length.out = 10),
        include.lowest = TRUE
      )
    ) |>
    group_by(varc, iter2) |>
